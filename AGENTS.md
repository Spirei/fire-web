...
- 每次成功推送后的交付答复另起一行使用引用格式：`> 已成功推送 \`短提交号\`，类型检查通过；简明说明本次更新内容。` 提交号前不加中文冒号；多项修改用一句话或分号分隔，不重复过程。只有 `git push` 明确成功且本次确实通过 `npx tsc --noEmit` 后才能写“类型检查通过”；未运行或未通过时必须如实说明，Actions 检查状态另行说明。
