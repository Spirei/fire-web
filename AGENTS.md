...
- 交付答复必须先用正常正文说明本次改了什么、为什么这样改、如何验证；有多个具体变化时使用简短项目符号逐项列出。**不得用推送回执代替修改明细，也不得为了压缩篇幅删掉用户需要的结果说明。** 正文之后再附两行引用回执：第一行固定为 `> ✅ 类型检查通过，已成功推送\`短提交号\``，“已成功推送”与提交号之间不留空格；第二行固定为 `> 📝 类型: 简明说明本次更新内容。` 两行之间不空行，提交号前不加中文冒号，类型必须与提交标题一致。例如：`> ✅ 类型检查通过，已成功推送\`39936ff\`` 换行 `> 📝 docs: 统一提交标题与推送回执格式。` **交付答复不再输出 GitHub Actions 自动检查状态**；用户主动询问时再查询并单独回答。只有 `git push` 明确成功且本次确实通过 `npx tsc --noEmit` 后才能写“类型检查通过”；未运行或未通过时必须如实说明。
