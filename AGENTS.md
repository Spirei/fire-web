...
- **提交标题统一使用规范类型前缀 + 简明中文摘要**，因为「部署状态」页会直接显示 GitHub 提交标题。前缀按改动性质选择：`feat:` 新功能、`fix:` 功能修复、`style:` 纯界面样式、`docs:` 文档或规范、`refactor:` 重构、`test:` 测试、`chore:` 维护；冒号后直接概括用户能感知的修改和结果，不中英夹杂，不使用生硬的自造词、职场黑话或比喻性动词。例如 `fix: 卡面库记住上次选择`、`style: 卡面详情去除重复 NEW 标记`。
- 每次涉及 Web 代码、样式或配置的修改，提交前必须运行 `npx tsc --noEmit`，通过后再提交；不得用 `git diff --check` 代替类型检查。纯文档改动无需重复运行类型检查，但交付时必须如实写明实际执行的检查。
- 每次成功推送后的交付答复另起一行使用引用格式：`> 类型检查通过，已成功推送 \`短提交号\`；简明说明本次更新内容。` 类型检查结果放在推送结果前面，提交号前不加中文冒号；多项修改用一句话或分号分隔，不重复过程。只有 `git push` 明确成功且本次确实通过 `npx tsc --noEmit` 后才能写“类型检查通过”；未运行或未通过时必须如实说明，Actions 检查状态另行说明。
