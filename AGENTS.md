...
- **提交标题统一使用中文分类 + 简明中文摘要**，因为「部署状态」页会直接显示 GitHub 提交标题。分类按改动性质选择：`新增功能`、`问题修复`、`样式调整`、`文档变更`、`代码重构`、`测试调整`、`日常维护`；分类与摘要之间使用 ` > `。标题不使用 `feat:`、`fix:`、`style:`、`docs:` 等英文前缀，不中英夹杂，不使用生硬的自造词、职场黑话或比喻性动词。例如 `问题修复 > 卡面库记住上次选择`、`样式调整 > 卡面详情去除重复 NEW 标记`。
- 每次涉及 Web 代码、样式或配置的修改，提交前必须运行 `npx tsc --noEmit`，通过后再提交；不得用 `git diff --check` 代替类型检查。纯文档改动无需重复运行类型检查，但交付时必须如实写明实际执行的检查。
- 每次成功推送后的交付答复另起一行使用引用格式：`> 类型检查通过，已成功推送 \`短提交号\`；中文分类 > 简明说明本次更新内容。` 类型检查结果放在推送结果前面，提交号前不加中文冒号；分类必须与提交标题一致，多项修改用一句话或分号分隔，不重复过程。例如：`> 类型检查通过，已成功推送 \`77f4c0e\`；文档变更 > 调整推送回执顺序，后续先写检查结果，再写推送结果和更新摘要。` 只有 `git push` 明确成功且本次确实通过 `npx tsc --noEmit` 后才能写“类型检查通过”；未运行或未通过时必须如实说明，Actions 检查状态另行说明。
