...
- 每次成功推送后的交付答复另起一行使用引用格式：`> 类型检查通过，已成功推送 \`短提交号\`；类型: 简明说明本次更新内容。` 类型检查结果放在推送结果前面，提交号前不加中文冒号；类型必须与提交标题一致，多项修改用一句话或分号分隔，不重复过程。例如：`> 类型检查通过，已成功推送 \`77f4c0e\`；docs: 调整推送回执顺序，后续先写检查结果，再写推送结果和更新摘要。` 下一行单独写 `> 自动检查：排队中／运行中／已通过／失败于具体步骤。` 只有 `git push` 明确成功且本次确实通过 `npx tsc --noEmit` 后才能写“类型检查通过”；未运行或未通过时必须如实说明。
