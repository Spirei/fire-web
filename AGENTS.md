...
- 每次成功推送后的交付答复使用两行引用：第一行固定为 `> ✅ 类型检查通过，已成功推送\`短提交号\``，“已成功推送”与提交号之间不留空格；第二行固定为 `> 📝 类型: 简明说明本次更新内容。` 两行之间不空行，提交号前不加中文冒号，类型必须与提交标题一致，多项修改用一句话或分号分隔，不重复过程。例如：`> ✅ 类型检查通过，已成功推送\`39936ff\`` 换行 `> 📝 docs: 统一提交标题与推送回执格式。` 自动检查状态另起一段写为 `> 🔄 自动检查：排队中／运行中／已通过／失败于具体步骤。` 只有 `git push` 明确成功且本次确实通过 `npx tsc --noEmit` 后才能写“类型检查通过”；未运行或未通过时必须如实说明。
