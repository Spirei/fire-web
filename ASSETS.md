...
Provide media under `public/uploads`, fonts under `public/fonts`, display icons under `public/icons`, illustration/card backgrounds under `public/images` and `public/share`, and an authorized map dataset at `public/maps-world.json`. Uploads are mounted at runtime; build contexts exclude these directories, so they can never be baked in by copying the source tree.
