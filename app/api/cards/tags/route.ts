...
import { listCardTags, saveCardTags } from "@/lib/cardAmounts";
import { clientIp, rateLimit } from "@/lib/rateLimit";

export const dynamic = "force-dynamic";

/** 卡面 key 归一化（与录入金额同一套：按路径分段 URL 编码，幂等） */
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ tags: listCardTags(user.id) }, { headers: { "Cache-Control": "no-store" } });
}

export async function PUT(request: Request) {
  const user = getAuthUser(request);
  if (!user) return NextResponse.json({ error: "未登录" }, { status: 401 });
  if (!rateLimit(`card-tags:${user.id}:${clientIp(request)}`, 120, 60 * 1000)) {
