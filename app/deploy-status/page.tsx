...
export default function DeployStatusPage() {
  ...
  const [config, setConfig] = useState({ repository: "", hasToken: false });
  ...
  return (
    <main className="min-h-screen bg-[#0b0f16] px-5 py-10 text-slate-100 sm:px-8">
      <div className="mx-auto max-w-3xl">
        ...
        {configOpen && <section className="mb-6 rounded-2xl border border-blue-400/30 bg-[#121923] p-5"><h2 className="text-sm font-medium">GitHub 发布配置</h2><p className="mt-1 text-xs text-slate-500">仅管理员可保存；Token 不会回显，也不会发送到浏览器。</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-xs text-slate-400">仓库（账号/仓库名）<input value={config.repository} onChange={(event) => setConfig({ ...config, repository: event.target.value })} className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-950/40 px-3 py-2 text-sm text-slate-200 outline-none focus:border-blue-400" placeholder="owner/repository" /></label><label className="text-xs text-slate-400">Token{config.hasToken ? "（已配置，留空保持）" : ""}<input value={token} onChange={(event) => setToken(event.target.value)} type="password" autoComplete="new-password" className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-950/40 px-3 py-2 text-sm text-slate-200 outline-none focus:border-blue-400" placeholder="只读 Actions Token" /></label></div>{configError && <p className="mt-3 text-xs text-red-300">{configError}</p>}<div className="mt-4 flex justify-end gap-2"><button onClick={() => setConfigOpen(false)} className="rounded-lg border border-slate-700 px-3 py-2 text-xs text-slate-400">取消</button><button onClick={() => void saveConfig()} className="rounded-lg bg-blue-400 px-3 py-2 text-xs font-medium text-slate-950">保存配置</button></div></section>}
