...
export default function DeployStatusPage() {
  ...
  const triggerPublish = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (response.status === 409) { setNotice(data.error || "已有手动发布正在运行"); void load(); return; }
  ...
  const publishLabel = manualPublishActive ? "发布进行中…" : triggering ? "推送中…" : "立即发布";
