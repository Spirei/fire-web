      ...
      return (
        <span className="flex min-w-[150px] items-center gap-2">
          <span className="relative flex-none">
            {icon ? <img src={icon} alt="" className="h-7 w-7 rounded-full object-cover" /> : <i className="flex h-7 w-7 items-center justify-center rounded-full bg-bg-gray not-italic">{record.name.slice(0, 1)}</i>}
          </span>
          <span className="min-w-0">
            <b className="block truncate">{record.name}</b>
            <small className="mt-0.5 flex min-w-0 items-center gap-1.5 text-muted">
              ...
              <QuoteRowHint market={record.market} quote={quote} quotes={quotes} />
            </small>
          </span>
        </span>
      );
            ...
            <div>{shownPnlPositions.map(({ record, pnl }, index) => {
              ...
              return <div key={record.id} className="grid grid-cols-[48px_minmax(0,1fr)_110px] items-center border-t border-edge px-4 py-3 text-xs transition-colors first:border-t-0 hover:bg-bg-gray/60 dark:hover:bg-[#1b2230]">
                ...
                <span className="flex min-w-0 items-center gap-2">
                  <span className="relative flex-none">
                    {icon ? <img src={icon} alt="" className="h-7 w-7 rounded-full object-cover" /> : <i className="flex h-7 w-7 items-center justify-center rounded-full bg-bg-gray not-italic text-ink-2">{record.name.slice(0, 1)}</i>}
                  </span>
                  <span className="min-w-0">
                    <b className="block truncate">{record.name}</b>
                    <small className="mt-0.5 flex min-w-0 items-center gap-1.5 text-muted">
                      ...
                      <EtfDoubleBadge market={record.market} code={record.code} name={record.name} />
                      <span className="truncate">{record.code}</span>
                      <QuoteRowHint market={record.market} quote={quotes[record.id]} quotes={quotes} />
                    </small>
                  </span>
                </span>
