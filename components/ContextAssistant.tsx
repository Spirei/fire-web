            ...
            {historyOpen && <div className="absolute inset-x-0 bottom-0 top-[68px] z-20 bg-white dark:bg-[#17191d]">
              <aside className="flex h-full w-full flex-col" aria-label="历史对话抽屉">
                ...
                <div className="flex-1 overflow-y-auto p-3">
                  {conversations.length === 0 ? <div className="flex h-full flex-col items-center justify-center px-8 pb-16 text-center"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-bg-gray text-muted dark:bg-white/[.06] dark:text-white/45"><IconMessageCircle size={22} /></span><div className="mt-4 text-sm font-semibold text-ink dark:text-white/85">还没有归档对话</div><p className="mt-1.5 text-xs leading-5 text-faint dark:text-white/35">开始一次对话后，它会自动保存并出现在这里。</p></div> : conversations.filter((conversation) => !historyQuery.trim() || conversation.title.toLowerCase().includes(historyQuery.trim().toLowerCase()) || conversation.id.toLowerCase().includes(historyQuery.trim().toLowerCase())).map((conversation) => <div key={conversation.id} role="button" tabIndex={actionBusy ? -1 : 0} aria-disabled={actionBusy} onClick={() => { if (!actionBusy) selectConversation(conversation); }} onKeyDown={(event) => { if (!actionBusy && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); selectConversation(conversation); } }} className={`group/history mb-1 flex w-full cursor-pointer items-center gap-3 rounded-xl px-3 py-3 text-left transition-colors ${actionBusy ? "cursor-not-allowed opacity-40" : ""} ${conversation.id === conversationId ? "bg-[#edf8f5] dark:bg-[#20332f]" : "hover:bg-bg-gray/80 dark:hover:bg-white/[.055]"}`}>
                    <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${conversation.id === conversationId ? "bg-white text-[#168b7c] shadow-sm dark:bg-white/10 dark:text-[#76c9bb]" : "bg-bg-gray text-muted dark:bg-white/[.05] dark:text-white/40"}`}><IconMessageCircle size={16} /></span>
            ...
            <>
              ...
              <form onSubmit={(event) => { event.preventDefault(); void send(input); }} className="border-t border-edge p-4 pb-[max(16px,env(safe-area-inset-bottom))]">
                <div className="flex items-end gap-2 rounded-[20px] border border-edge-strong bg-bg-gray p-2 pl-4">
                  ...
                  <button type="submit" disabled={!input.trim() || loading} className="assistant-send group flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-[#c5eee5] bg-[#e8faf6] text-[#0d9488] shadow-[0_4px_14px_rgba(15,159,143,.1)] transition-all hover:border-[#9ee3d5] hover:bg-[#d9f7f0] active:scale-95 disabled:opacity-30 dark:border-[#28534b] dark:bg-[#203b36] dark:text-[#7ed7c7] dark:hover:border-[#397065] dark:hover:bg-[#294a44]" aria-label="发送"><IconArrowUp size={19} className="transition-transform duration-200 group-hover:-translate-y-0.5" /></button>
                </div>
