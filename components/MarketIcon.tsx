...
function VectorFallback({ size, className, title }: { size: number; className: string; title?: string }) {
  return (
    <span className={`inline-flex flex-none items-center justify-center overflow-hidden rounded-full bg-bg-gray ${className}`} style={{ width: size, height: size }} title={title} aria-hidden>
...
export default function MarketIcon({
  ...
  return <VectorFallback size={size} className={className} title={title ?? (code || market)} />;
}
