      ...
      <div className="stock-chart-ranges">
        ...
        <div className="stock-chart-popover-wrap period-picker-wrap">
            ...
            {periodOpen && <div className="stock-chart-menu period-menu" role="menu" aria-label="K 线周期">
              ...
              <section><h4>分钟</h4><div className="period-option-grid">{[1,2,3,5,10,15,20,30,45].map((minutes) => <button key={minutes} type="button" className={range === "DAY" && intradayMinutes === minutes ? "is-selected" : ""} onClick={() => { setIntradayMinutes(minutes); setRange("DAY"); setPeriodOpen(false); }}>{minutes}分</button>)}</div></section>
              <section><h4>小时</h4><div className="period-option-grid">{[60,120,180,240].map((minutes) => <button key={minutes} type="button" className={range === "DAY" && intradayMinutes === minutes ? "is-selected" : ""} onClick={() => { setIntradayMinutes(minutes); setRange("DAY"); setPeriodOpen(false); }}>{minutes / 60}小时</button>)}</div></section>
              <section><h4>长周期</h4><div className="period-option-grid">
