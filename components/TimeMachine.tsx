...
export function TimeMachineLink({ to, iconOnly = false }: { to: "simple" | "full"; iconOnly?: boolean }) {
  return <a href={to === "simple" ? "/simple-app" : "/records"} data-time-machine={to}
    className={`fire-time-link${iconOnly ? " is-icon" : ""}`} aria-label={`穿越至${to === "simple" ? "简化版" : "完整版"}`} title={`时光机 · 穿越至${to === "simple" ? "简化版" : "完整版"}`}>
    <span className="fire-time-link-orbit" aria-hidden="true"><IconBrandTelegram size={19} stroke={1.45} /></span>
    {!iconOnly && <><span className="fire-time-link-label">去另一面</span><span className="fire-time-link-destination">{to === "simple" ? "简化版" : "完整版"}</span></>}
  </a>;
}
