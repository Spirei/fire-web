...
export default function ActivitiesView({ activities, systemLogs = [], isAdmin = false, onRefresh }: Props) {
  ...
  return (
    <div className="overflow-hidden rounded-[18px] border border-edge bg-white shadow-card dark:bg-[#151b26]">
      ...
      {scope === "user" && dailySummary && <div className="mx-5 mt-4 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-edge bg-bg-gray/50 px-4 py-3"><div><p className="text-xs font-semibold text-muted">前一交易日盈利摘要</p><p className="mt-1 text-[11px] text-faint">{dailySummary.date} · {dailySummary.settlement}</p></div><div className="flex items-center gap-5"><div><p className="text-[11px] text-muted">已实现盈亏</p><strong className={`text-base tabular-nums ${dailySummary.realized >= 0 ? "text-up" : "text-down"}`}>{dailySummary.realized >= 0 ? "+" : "−"}{Math.abs(dailySummary.realized).toFixed(2)} <span className="text-[11px] font-normal">{dailySummary.currency}</span></strong></div><div><p className="text-[11px] text-muted">成交笔数</p><strong className="text-base tabular-nums text-ink dark:text-white">{dailySummary.trades}</strong></div></div></div>}
