...
export default function ActivitiesView({ activities, systemLogs = [], isAdmin = false }: Props) {
  ...
  return (
    <div className="card overflow-hidden">
      ...
      <div className="grid grid-cols-2 gap-2 border-b border-edge px-4 py-3 sm:grid-cols-4"><div className="rounded-lg bg-bg-gray px-3 py-2"><p className="text-[11px] text-muted">用户事件</p><strong className="text-lg tabular-nums">{activities.length}</strong></div><div className="rounded-lg bg-bg-gray px-3 py-2"><p className="text-[11px] text-muted">系统事件</p><strong className="text-lg tabular-nums">{systemLogs.length}</strong></div><div className="rounded-lg bg-bg-gray px-3 py-2"><p className="text-[11px] text-muted">今日活动</p><strong className="text-lg tabular-nums">{activities.filter((a) => a.createdAt.slice(0, 10) === new Date().toISOString().slice(0, 10)).length}</strong></div><div className="rounded-lg bg-bg-gray px-3 py-2"><p className="text-[11px] text-muted">当前显示</p><strong className="text-lg tabular-nums">{scope === "user" ? visibleActivities.length : visibleSystemLogs.length}</strong></div></div>
      <div className="border-b border-edge px-4 py-3"><input value={query} onChange={(e) => setQuery(e.target.value)} aria-label="搜索日志" placeholder="搜索用户、股票、事件或详情…" className="h-9 w-full rounded-lg border border-edge bg-transparent px-3 text-sm outline-none placeholder:text-faint focus:border-brand sm:max-w-sm" /></div>
