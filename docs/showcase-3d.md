...
3. **模型调校工作台**：左侧大画布使用模型展示同款探索镜头，可全角度环视，以 Shift / 中键 / 右键平移、滚轮推进、双击局部聚焦，并在长参数列表滚动时保持吸顶可见；右侧修改车长（归一化比例）、绕 Y / X 朝向修正、轮子材质与旋转轴（勾中的材质会自转）、贴图上限及尾翼线框算法。参数修改后点“重新加载预览”，确认结构与轮子识别无误。
