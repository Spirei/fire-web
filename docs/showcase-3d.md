...
首页右上角网格立方体进入独立浅灰（#e7e7e7）模型画布，临时隐藏环境几何与首页 HUD，直接渲染跳过后期和倒影。独立自由镜头缩放上限为 80 倍初始距离，far=5000 避免缩小时裁掉模型；关闭 / Escape 恢复原自由镜头、旋转、焦点和缩放。复位恢复查看器初始机位。显示面板：原生、叠加线框、纯线框。六个色块直接切换到对应颜色的叠加模式；纯线框在浅色背景使用深色，在深色背景使用浅灰。模式属于当前展示会话，换车时沿用、刷新回到原生。
