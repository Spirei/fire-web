...
登录后在首页右下角车型条末尾点「＋」，或直接开 `/showcase/import`：
...
- 冲刺隧道（屏幕空间光条）参照的是本地参考片 `~/Downloads/0.临时/素材/迈凯轮/3D/0919.mp4`（拍的是官方站屏幕；同目录的 `NUMB.mp4` 是那首歌的 MV，不是参考片）。参考在高速段（321 / 334 km/h）是「一圈细长线 + 密集短段」：单根 0.4-0.5°（1600 宽下约 6-8px）、一条线上 5-6 段、金（外墙）与冷白（近水平那几条）两色，收敛于消失点；138 km/h 只有零星几根淡线，000 km/h 完全没有。参数都在 `speed.tunnel`：`bars`（主光条角度 / 宽度 / 冷暖）、`lanes`（跑道边线与地面标线）、`auxCount` / `auxOpacity`（壁面细线密度与亮度）、`barSegment`（段长系数，越大段越短）、`barIntensity`；渐显曲线在引擎里（`lightLinesPass.uniforms.uStrength`，速度三次方）。
