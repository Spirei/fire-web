...
以**管理员**登录后，先看 `/showcase/pipeline` 的七步全链路，再从首页右下角车型条末尾点「＋」，或直接开 `/showcase/import`（普通用户看不到入口、接口也会 403）：
...
2. **体检报告**：服务端会列出导出器、顶点 / 三角面 / 节点 / 网格 / 材质 / 贴图数量与最大贴图尺寸，并给出建议；外链缓冲区或贴图会被拒收，非三角图元、超高面数和超多节点会明确提示；
   Draco / GPU 实例化会被拒收；KTX2、Meshopt、WebP 已有显示支持，可以通过体检。不要引用 GLB 外部的缓冲区或贴图。
...
- 显示副本使用 **KTX2 / UASTC level 3、关闭 RDO、Zstd 18**；不缩贴图、不减面，原始文件保留。超过 50 MiB 的大模型会尝试 Meshopt 几何量化，仅在至少减少 20% 文件大小且贴图尺寸不变时采用；拓扑不变但顶点坐标并非逐位无损。UASTC 也属于高质量有损，不能称逐像素无损。下载 gzip/Brotli 或 PNG 优化无法减少原始像素/GPU 驻留内存。
...
- 内存受限设备也保留当前车直到新车解析完成；成功后才释放旧车并替换，失败时保留旧车。跨场景串行加载、逐张解码、销毁 ImageBitmap、释放过期结果；GPU 压缩纹理直接上传，不画到 Canvas 解压。原画画布仍保留最高 3 倍像素比，各设备恢复原有的 800 万输出像素上限。双车在切换瞬间的峰值内存仍需实体设备验证。
...
流程页 `/showcase/pipeline` 可选择「本机工具」或「群晖处理」。本机方式仍在 `tools/model-optimizer` 目录执行 `npm start`，Mac 与 Windows 共用本地网页：选择原始 GLB，完成后下载结果。在线方式把已保存的原始 GLB 放入 uploads 卷的持久化队列，由独立 `fire-model-worker` 容器串行生成附件，主网站进程不执行编码。两种方式都导出 1K 首页预览，大模型按原有阈值生成保留贴图尺寸的 UASTC 高清副本，原文件保留。群晖容器限制 2 CPU / 8 GiB，使用同一 uploads 卷且无网络/对外端口；取消、失败可在流程页重试，旧副本保留一份回退。

本机方式：先在网站导入同一份原始 GLB，保存后在同车型卡点击“上传首页预览”，选择 `*-preview.glb`；仅本地报告显示已压缩时，再点“上传移动端原画”选择 `*-optimized.glb`。群晖方式：保存新车型后自动排队，也可给已存车型单独点击“群晖生成附件”，成功后附件直接挂载，无需重复上传。主网站只做管理员鉴权、大小限制、GLB 结构体检与队列管理，不调用编码器。KTX2/Meshopt/WebP 已有显示支持，因此体检接受这些格式；仍拒绝未支持的 Draco、GPU 实例化及外链资源。

网站移除生成接口、轮询和编码工具依赖。保留 Three.js 的 KTX2/Meshopt 读取能力与已有 GPU 副本读取兼容，避免影响已上线车型。编码器和浏览器解码器作用不同，后者不能删除。Next.js 自身使用的 sharp 也不随编码器一起删除。

所有压缩从原始 GLB 开始，避免重复有损压缩；导出文件夹含报告和上传说明。GPU 压缩版保留源尺寸，不保证逐像素无损或下载体积更小。详细操作见 `tools/model-optimizer/README.md`。
