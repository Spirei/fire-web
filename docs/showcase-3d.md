...
先在网站导入同一份原始 GLB，保存后在同车型卡点击“上传首页预览”，选择 `*-preview.glb`；仅本地报告显示已压缩时，再点“上传移动端原画”选择 `*-optimized.glb`。网站只做管理员鉴权、大小限制、GLB 结构体检与保存，不调用编码器。KTX2/Meshopt/WebP 已有显示支持，因此体检接受这些格式；仍拒绝未支持的 Draco、GPU 实例化及外链资源。
