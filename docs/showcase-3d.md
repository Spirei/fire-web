...
首页右上角网格立方体进入独立浅灰（#e7e7e7）模型画布，临时隐藏环境几何与首页 HUD，直接渲染跳过后期和倒影。独立自由镜头缩放上限为 80 倍初始距离，far=5000 避免缩小时裁掉模型；关闭 / Escape 恢复原自由镜头、旋转、焦点和缩放。复位恢复查看器初始机位。显示面板：原生、叠加线框、纯线框。叠加与纯线框共用黑、浅灰、红、蓝、绿、黄六色；换色保持当前线框模式，原生模式点色块才进入叠加。所选颜色跨模式沿用，不再由背景主题覆盖。模式属于当前展示会话，换车时沿用、刷新回到原生。
