...
3. **独立模型调校工作台**：进入改参数后打开全屏工作区，顶部常驻保存与返回。左侧大画布使用模型展示同款探索镜头；整体、车身、尾翼、轮组、座舱使用不同颜色的三维投影点，随模型环视和缩放贴在对应部位。单击标注点或真实网格选择参数大类，所选大类保留彩色线框、其余网格退成低透明灰线；双击具体零件会在镜头聚焦的同时下钻到单个网格。右侧独立滚动，可修改车长、朝向、轮子、贴图及尾翼线框算法；共面三角边清理与折角阈值可为每台车单独保存。停止输入 350 毫秒后自动重建预览。
