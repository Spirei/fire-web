...
1. **选文件**：拖入单个 `.glb`（≤ 250MB）。只放模型文件，贴图要已经打进 glb。上传通过体检后先保存为隐藏草稿，不会提前进入首页；未保存草稿超过 24 小时会自动清理。
...
3. **摆正与认轮子**：预览里改车长（归一化比例）、绕 Y / X 朝向修正、轮子材质与旋转轴（勾中的材质会自转）及贴图上限。参数修改后点“重新加载预览”，确认结构与轮子识别无误。
4. **保存并上线**：只有预览成功且当前参数已经重载确认时才能保存；草稿原子转成正式文件并写进 `public/uploads/mclaren/showroom.json`，首页车型条立刻多一辆车。
