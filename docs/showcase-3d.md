...
- 线框展示对所有导入车型走同一套自适应覆盖：已有密集拓扑直接复用；端板、贴花面等长边稀疏三角形只生成展示用细分几何，不修改原模型。单网格 ≥ 20 万三角形不再细分，整车新增三角形预算为 100 万；换车 / 退出时释放生成几何。蒙皮或 Morph 模型保留原始拓扑，避免丢失顶点权重；`EXT_mesh_gpu_instancing` 在上传体检阶段拒绝，要求先转普通网格。
