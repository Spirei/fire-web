...
export const V0_1_19_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "通过 iPhone 12 Pro 视口定位图表工具栏左侧周期组被 Flex 压缩至 41px 的问题；手机端改为周期与图表操作上下分区，K 线周期在自身区域平滑横滑，复权、比较、图形和指标按钮保持完整文字，极窄屏允许操作组自然换行。顶部行情指标改为默认隐藏，可按需显示为 2×2 卡片；时段、周期、复权、比较、图形和指标选择框统一使用视口内底部安全面板，解决弹层越界；≤420px 时按视口收敛 K 线高度，避免窄屏图形纵向拉长；详情头部同步缩小返回键与图标、截断过长名称、隐藏重复代码并收紧价格排版。",
    kind: "fix"
  }]
};
