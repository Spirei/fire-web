...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "直接参照 DeepSeek Harness 开源源码移植完整智能助手界面：工作台采用固定会话侧栏、单一会话画布与底部输入坞；输入框复刻 22px 圆角、浮层模型/数据菜单、工具行、状态按钮及弹出动画；会话右侧增加同款轮次短线轨道，滚动时高亮当前轮，悬停显示问题摘要，点击平滑跳转。新增同结构的 800×800 设置中心、188px 设置导航、32px 圆角、遮罩模糊和进场动画，包含通用设置、模型、插件和助手预设；主题、字号及对话密度可持久化。轨迹页保留 Input / Model / Tools 三泳道、高密度调用列表及右侧详情。功能映射现有模型服务、自动回退、图片、记忆与数据范围；保留本站全局导航，按要求排除余额用量卡。记录按用户和对话隔离，默认保留 30 天，删除对话同步清理，且不保存 API Key、请求头、账户摘要、问题正文、回答正文或图片内容。",
    kind: "feature"
  }, {
    title: "新增智能助手独立工作台",
