...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "除独立智能助手工作台外，主站页面统一增加 Fire / 当前页面面包屑；持仓页顶栏显示我的持仓，页面内部资产卡片沿用账户资产，资产分析、FIRE 与全球经济显示对应四色门状态。顶栏与内容归入同一个圆角工作区外壳，内边距按可用宽度自适应，不修改持仓表格、资产分析分栏、设置窗口等页面内部列宽，并完成深浅色适配；矮窗口同步增加手势与侧栏分隔线的间距。",
    kind: "feature"
  }]
};
