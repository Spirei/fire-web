...
export const V0_1_25_ENTRY: VersionEntry = {
  ...
  changes: [{ title: "双向版本时光机", desc: "简化版工具栏与完整版页头新增版本穿越入口，点击即以用户提供的哆啦A梦时光机原图衔接页面切换，以 WebGL 对原图做柔和纵深与局部视差，保护人物区域，无额外流光或闪光，不支持 WebGL 时回退原图；支持深浅主题、手机小屏、减少动态效果，通过预取和应用内路由消除整页重载；隔离简化版样式并恢复返回后的窗口交互，离开前等待账本同步，失败时保留当前页面。", kind: "feature" }, { title: "版本穿越入口布局", desc: "简化版右上角直接显示无外框细线纸飞机，单击立即穿越；完整版入口保留在头像菜单，配合「去另一面」及目标版本说明。", kind: "fix" }, { title: "证券市场色块全站统一", desc: "为持仓、自选、资产分析排行、全球榜单、个股详情、公司资料、相关 ETF、交易弹窗、名人持仓、素材库和 K 线对比中的证券名称/代码组合补齐统一市场色块；美股、港股分别显示 US、HK，A 股按代码显示 SH 或 SZ，保留已正确实现的位置，并排除货币、文件、订单号等非证券代码。", kind: "feature" }, { title: "卖出回款正确进入账户余额", desc: "修复历史买入缺少期初入金时，负现金净额会吞掉后来真实卖出回款的问题。现金余额改为按币种和发生时间逐笔计算，并在每一步执行零余额下限；SPCH.AM 卖出 266 股的 2,779.70 美元现会完整进入美元余额，总资产再按持仓市值加余额汇总。账户概览以“余额”替换不准确的“最大购买力”，并保留可用现金与冻结现金口径。", kind: "fix" }, { title: "修复资产分析总资产闪现", desc: "资金余额请求完成前不再用持仓市值加 0 余额渲染总资产，避免刷新时先闪现约 5.8 万的中间值；余额加载完成后再展示净资产、余额与可用现金，持仓市值等独立指标保持即时显示。", kind: "fix" } , { title: "重排资产分析账户指标", desc: "按参考图重排账户资产卡片：将当日盈亏移到总资产右侧，并与下方现金指标右对齐；下方统一展示持仓总市值、持仓总盈亏和现金。现金继续等待余额加载完成后显示，避免出现不完整中间值。", kind: "fix" } , { title: "优化资产分析区块命名", desc: "将账户资产卡片下方的持仓列表区块命名为“持仓分布”，更准确对应市场筛选、持仓构成与列表内容。", kind: "fix" } , { title: "优化账户预览标题", desc: "将账户总盈亏区块标题改为“账户预览”，与区块内的净资产、当日盈亏、持仓和现金概览内容保持一致。", kind: "fix" } , { title: "调整账户区块标题", desc: "将资产分析区块标题从“账户预览”调整为“账户总览”，与整体账户指标汇总语义保持一致。", kind: "fix" } , { title: "为持仓分布增加分页", desc: "持仓分布列表按每页 10 项分页，保留市场筛选、搜索和排序，并在切换条件时回到第一页，避免长列表撑开页面。", kind: "feature" } , { title: "统一持仓分布分页样式", desc: "持仓分布分页改用全站统一 Pagination 组件，提供页码窗口、上一页/下一页与跳转输入，并保留市场筛选、搜索和排序。", kind: "fix" }, { title: "统一 A 股市场色块文字颜色", desc: "全站 A 股 SH/SZ 市场色块改用白色文字，与美股 US、港股 HK 及其他市场徽标保持一致，提升粉红底色上的可读性。", kind: "fix" }]
};
