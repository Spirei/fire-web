...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "直接参照 DeepSeek Harness 开源源码移植完整智能助手界面：独立工作台采用固定会话侧栏、单一会话画布与底部输入坞；输入框复刻 22px 圆角、浮层模型/数据菜单、工具行、运行统计、状态按钮及弹出动画。浮动入口恢复最初的轻量面板，小输入区只保留完整工作台入口和发送按钮，隐藏模型等高级控件；发送按钮的空闲、可发送、回答中停止及回答中再次输入状态统一使用 ChatGPT 风格绿色体系。工作台或助手设置中选择的默认模型写入账户偏好，并同步用于各页面浮动入口。会话右侧增加轮次短线轨道。新增 800×800 设置中心，包含通用设置、对话与记忆、模型、插件和助手预设；空间、记忆和模型调用统计集中进入设置。空间选择采用自定义浮层。会话列表支持重命名、归档、恢复和删除，手动标题不会被后续保存覆盖。工作台、浮动面板、设置中心及其浮层统一服从助手自身主题。模型服务可在当前弹窗内选择和编辑，全程不跳转。轨迹页保留 Input / Model / Tools 三泳道、高密度调用列表及右侧详情。记录按用户和对话隔离，删除对话同步清理，且不保存 API Key、请求头、账户摘要、问题正文、回答正文或图片内容。",
    kind: "feature"
  }, {
    title: "新增智能助手独立工作台",
