...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "新增智能助手独立工作台",
    desc: "左侧导航在财报日历下方新增智能助手入口，提供完整对话工作台：沿用既有服务端会话、归档、模型选择、数据范围、临时对话、图片策略、记忆、空间、调用统计、流式回答、停止生成、编辑分支、重试与导出能力。原右下角可拖动入口保持，浮层收敛为随时提问；标题栏新增进入完整工作台的快捷入口。独立页适配深浅色与手机宽度，当前页不重复挂载第二个浮动实例，避免会话状态竞争。TypeScript、站点 Review 与生产构建通过。",
    kind: "feature"
  }]
};
