...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "全站货币国旗首屏提速",
    desc: "货币国旗继续全部以素材库为配置来源，但不再由每个 CurrencyFlag 实例触发完整 250 个国家/地区素材请求。服务端首屏只读取固定的 7 种展示货币并注入共享缓存，同时预载当前显示货币国旗；资金系统的扩展币种只在组件实际出现时按 ISO 代码请求。我的持仓、资产分析、资金系统、FIRE、名人持仓和卡面库统一受益，素材库替换国旗后仍全站生效。全球经济因需要完整国家目录，继续按需读取全部国旗。",
    kind: "fix"
  }]
};
