...
export const V0_1_27_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "新增收益曲线前后对比页（/mockups/curve-compare.html）",
    desc: "把「改动前（ef550b6 版 runtime）」与「现在（lib/curve.ts + lib/curveSvg.ts）」并排渲染成可点击对比页，全部用合成数据：① 常规近 6 月（两版一致，只有边界日差一天）；② 3/31 选「近 1 月」（旧实现 setMonth 越界成 3/3，起点正好落在回撤最低点，曲线开头凭空跳涨 7.5%；新实现从 2/28 起，回撤与修复完整可见）；③ 基准行情晚于组合首日（旧实现把窗口内 80 天基准按自己的点数拉伸铺满整张图，时间轴错位；新实现按日期对齐、之前回填首值）；④ 五年日线 1825 点（命中圆点 1825 → 182，画面一致）；⑤ 累计收益曲线（末端对齐当前账面口径）。页面同时给出「周期窗口对照表」与「同一近 6 月、不同时刻算出的窗口起点」两张表，说明旧口径受时刻影响、新口径与时刻无关。旧侧由 vm 沙箱跑 git 里的真身，新侧直接调用当前 TS 模块，数字全部来自真实渲染结果。",
    kind: "feature"
  }, {
    title: "前后对比页接入真实账本（/curve-compare，需登录）",
    desc: "静态对比页用的是合成数据，看不出自己账户的真实差异 —— 新增登录保护的诊断页 /curve-compare：与简化版账户页同一个账本接口（/api/v1/simple-ledger）、同一份数据，左侧复刻改动前（ef550b6）口径、右侧当前实现，两侧都交给同一个 SVG 适配器渲染，所以画面差异只来自口径本身。可切账户 / 央行基准（与简化版同一份列表）/ 周期 / 收益率-累计收益口径，并把「旧实现的时钟」切到 00:30 / 08:30 / 12:00 / 23:00，直接看到旧实现窗口随时刻漂（新实现不动）；顶部差异条给出窗口起点、采样点、命中圆点、基准线点数、区间收益率的前后对照。旧口径复刻独立放在 lib/legacyCurve.ts（注释声明业务代码禁止引用），并用 132 组用例（6 周期 × 4 时刻 × 口径/基准）与 git 真身逐值核对：窗口、序列数值、基准序列完全一致，基准折线几何在 SVG 0.1 单位取整精度内一致（最大偏差 0.083 单位 ≈ 0.2px）。冒烟测试页面巡检加入 /curve-compare（未登录 307 跳转登录），113 → 114 项。tsc 无错误、npm run build 通过、冒烟 114/114 全 PASS。",
    kind: "feature"
  }]
};
