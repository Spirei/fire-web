...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "智能助手新增模型运行记录",
    desc: "参考 DeepSeek Harness 的追加式会话事件思路，为智能助手增加“对话 / 运行记录”视图。每次模型尝试记录实际服务、模型、尝试顺序、回退、总耗时、首字延迟、Token、图片数量、数据范围、状态和脱敏错误；支持按模型或状态搜索并查看单次详情。记录按用户和对话隔离，默认保留 30 天，删除对话时同步清理；不保存 API Key、请求头、账户摘要、问题正文、回答正文或图片内容。",
    kind: "feature"
  }, {
    title: "新增智能助手独立工作台",
