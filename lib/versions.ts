...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "对话右侧的轮次短线按「从上到下由旧到新」，但打开对话时高亮却固定停在第一轮（最上面），看起来像最新一轮在上面。现在打开对话或新增消息后高亮自动落到最新一轮（最下面那条）。样式与动效参考主流大模型软件的对话轨道：右对齐短横线、每根长度按 8 根一组的平缓旋律不同（12–20px）、当前轮更亮且只比邻条略长 4px（悬停多 8px，不再拉成 32px 的长条），静止时完全不动，只在悬停 / 切换时走 150ms 过渡，悬停弹出该轮内容预览。悬停预览从按钮左侧展开，轨道本身不做裁剪，预览不会被切掉。",
    kind: "fix"
  }]
};
