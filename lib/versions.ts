...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "对话右侧的轮次短线按「从上到下由旧到新」，但打开对话时高亮却固定停在第一轮（最上面），看起来像最新一轮在上面。现在打开对话或新增消息后高亮自动落到最新一轮（最下面那条）。样式参考主流大模型软件的对话轨道：右对齐短横线、当前轮更亮且只比邻条略长一点（2px + 4px，不再拉成 32px 的长条）、悬停出预览。动效收敛为「音量条呼吸」：每根基准长度按 8 根一组的平缓旋律不同（12–20px），以 3.2 秒的缓入缓出轻微伸缩、相位沿轨道推移，不再整体上下弹跳；系统「减少动态效果」时完全静止。轮次很多时导航高度收在面板内、从顶部裁掉最早的几条，保证最新一轮始终可见。",
    kind: "fix"
  }]
};
