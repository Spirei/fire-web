...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "除独立智能助手工作台外，主站页面统一增加当前页面顶栏；移除重复的 Fire / 前缀，持仓页显示我的持仓，页面内部资产卡片沿用账户资产，资产分析、FIRE 与全球经济显示各自页面名及对应四色门状态。业务内容恢复原有开放式页面布局，不再强制套用统一圆角外壳，保持持仓宽表、资产分析分栏与管理页面原有空间；矮窗口补足手势与侧栏分隔线的间距，智能助手按实际视口高度展示完整输入区。",
    kind: "feature"
  }]
};
