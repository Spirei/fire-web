...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    title: "登录二次验证（TOTP）",
    desc: "在设置 → 二次验证中扫描本地生成的二维码绑定验证器（密钥不经过第三方图片接口），输入 6 位数字后开启，并一次性给出 8 条备用码。开启后网页与 iOS 登录在密码正确时只返回 5 分钟有效的 ticket，因此需要再提交验证码或备用码才能登录。同一验证码不能重复使用；管理员重置密码会同时关闭该用户的二次验证，用户管理页也可以单独关闭。",
    kind: "security"
  }, {
    title: "二次验证改为独立设置项",
    desc: "从设置 → 个人信息中拆分出来，在账号分组下单独成项，地址为 ?sub=totp。个人信息仍然支持在改密时填写验证码。",
    kind: "fix"
  }, {
    title: "二次验证安全加固",
    desc: "备用码加长到 64 位熵并用数据密钥做 HMAC 存储，TOTP 密钥以 AES-GCM 加密后写入数据库。登录 ticket 与绑定过程改存 SQLite，因此多进程不会丢失；同一账号只保留最新一张 ticket，验证码连续错 8 次即作废。开启二次验证会注销其它会话；改密与管理员关闭二次验证都需要再次验证身份。用户列表不再查询密钥字段。",
    kind: "security"
  }, {
    title: "四色门两套样式各用各的音效",
    desc: "样式 2 珐琅盘使用八音盒音效（转动前 C-G-C 拇指琴、转动中五声音阶、停下时 C-G 五度）；样式 1 原插画转盘改回棘轮声（三角波 690→410Hz，停下时为 G6 谐波钟加 4.2kHz 轻响）。切换样式后下一次转动改用对应音效，挂钩与停止位置不变。",
    kind: "fix"
  }, {
    title: "发布状态热力图按全年提交计数",
    desc: "此前只统计最近 100 条提交，因此八月与九月前半被截断。现在按页读取全年 main 提交：1–7 月没有提交，8 月 30–31 日共 73 次，9 月 1–18 日共 767 次。格子外观仍与线上版一致，按周排列。",
    kind: "fix"
  }, {
    title: "后台页签恢复按需加载",
    desc: "持仓、自选股与资产分析仍然随外壳同步渲染（服务端首帧即有内容），其余页签改回 next/dynamic 按需下载，并且不设置 loading 挡板，因此刷新时不会再把已经画出的页面覆盖成「加载中…」。首屏空闲后再预取其余页签，首次进入也更快。",
    kind: "fix"
  }, {
    title: "四色门图片压缩体积",
    desc: "窗户 window.png 保持 838×447 像素，只做无损压缩。转盘 dial.png 从 2088×2068 改为 414×410（侧栏显示尺寸的 3 倍，覆盖三倍屏），使用 Lanczos 缩放并保留透明通道。指针与手型图本来就小，未作改动。",
    kind: "fix"
  }, {
    title: "图表与浮动助手改为按需加载",
    desc: "核心图表包去掉 MapChart 与默认捆绑的 SVG 渲染器，全球经济热图单独走 lib/echarts-map.ts，因此进入持仓与自选股时不再加载世界地图。浮动助手改为 next/dynamic，首屏空闲后再挂载，不占用持仓首屏时间。图表与助手外观不变。",
    kind: "fix"
  }, {
    title: "交易广场新增帖子评论与评论者头像",
    desc: "新增 lib/tradingSquareComments.ts，用配置中的雪球 Cookie 抓取评论并按帖子 id 存入 data/duan-comments.json，评论数与缓存时间都没有变化时跳过；一级页面只在帖子底部增加一个「评论 N」按钮。同时修复评论者头像取不到、被引用的帖子没有作者头像、正文中的「回复@某人:」被清除后缺少上下文这三处问题。",
    kind: "feature"
  }, {
    title: "交易广场卡片不再自行移动到中间",
    desc: "面板由 mx-auto 居中并用 translate 偏移，位置保存在 localStorage。原逻辑会把限制到内容区后的偏移写回存储，因此内容区变窄（更换显示器、窗口缩小）后卡片永久停在中间；现在限制只作用于渲染，只有拖动结束才写入位置，并增加窗口 resize 与内容区 ResizeObserver 监听。",
    kind: "fix"
  }, {
    title: "交易广场正文清除抓取带入的页面文案",
    desc: "新增 stripTradingSquareChrome() 并在 normalizeTradingText() 中统一调用，因此「回复@某人:」「查看图片」「网页链接」这类链接文字不再作为正文显示（275 条段永平帖中有 171 条带该前缀）。正文中间的「//@某某:」转发链与「[已修改]」等原帖标记保留。判断是否为回复改为在清洗前记录 DuanPost.reply 标记。",
    kind: "fix"
  }, {
    title: "评论改为回复流样式",
    desc: "评论不再套用圆角卡片，改为主流社交网站（X / Reddit / YouTube）的回复流，顶部一条细分隔线加「评论 N · 全部评论 ↗」，每条评论为「头像 + 名字 · 时间 + 正文 + 赞数」，楼中楼标出「回复 @某人」。一级页面仍然只显示「评论 N」按钮，点击后才展开。",
    kind: "feature"
  }, {
    title: "评论按数量分流显示",
    desc: "不超过 10 条时在帖子下直接展开，超过 10 条时「评论 N」改为入口，点击后进入二级评论页（原帖 + 返回按钮 + 完整评论列表），因此一级页面保持简洁。评论抓取从只取第一页改为评论超过 20 条的帖子最多取 3 页（60 条）。同时修复评论头像与名字行没有对齐的问题。",
    kind: "feature"
  }, {
    title: "修复特朗普动态不再更新",
    desc: "归档站改版后日期改为包在 <time datetime> 中、正文容器增加属性，两处旧正则因此静默失效，整页解析为 0 条。现在日期优先读取 datetime 属性，正文改为按 div 深度配对提取；单页超时从 4 秒放宽到 8 秒并按页容错，因此单页失败不会导致整次刷新没有任何写入。实测缓存从 291 条增至 311 条。",
    kind: "fix"
  }, {
    title: "修复交易广场更新延迟与刷新回退到旧帖",
    desc: "段永平转发别人的帖子时正文只有一个 emoji 图片，被 mapDuanStatus 判定为空而丢弃，因此保留条件改为「正文 / 图片 / 引用 有一个就保留」，并在去标签前取出 emoji 的 alt（显示为 [很赞]）。刷新时客户端曾用 localStorage 缓存整份替换服务端首帧数据，因此先显示旧帖再跳回新帖；现改为合并，服务端首帧优先。",
    kind: "fix"
  }, {
    title: "交易广场新增「新」角标",
    desc: "判断口径与作者卡片的未读数共用 lib/tradingSquareSeen.ts 的同一个纯函数，因此两处不会出现不一致；角标按进入页面时的已读快照计算，所以浏览过程中不会消失。评论时间同时改为相对时间（刚刚 / N 分钟前 / 昨天 HH:MM 等），点赞前增加拇指图标，二级评论页移除内层滚动条。",
    kind: "feature"
  }, {
    title: "新动态角标改用站内未读色",
    desc: "原来的实心绿胶囊使用 #4caf58，这个颜色不在站内色板内，而且被 ml-auto 推到行尾后与发布时间之间留有大段空白。改为站内未读色的软底小胶囊（浅色 bg-up-bg 配 text-up，深色 #3a1c1c 配 #ff8a8a），紧跟在发布时间之后。",
    kind: "fix"
  }, {
    title: "新动态标记改为主流做法",
    desc: "单条新动态在最前面显示 6px 圆点（浅色 #e23d3d、深色 #ff8a8a，与头像未读计数同色），并在新动态与上次看过的帖子交界处显示一次「以上 N 条为新动态」分隔线，因此不再逐条添加文字角标；分界位置由 lib/tradingSquareSeen.ts 的 unseenBoundaryIndex 计算，整页都是新动态或全部看过时不显示分隔线。",
    kind: "fix"
  }, {
    title: "未读分界标签改为压在帖间分隔线上",
    desc: "上一版的左对齐红色文字与横贯整行的红色细线，在深色模式下位于帖间两条分隔线之间，观感杂乱。现在改用微信「以下是新消息」与 Telegram 的写法：把一行 10px 中性灰小字居中压在原有的帖子分隔线中间（容器高度为 0，标签用卡片底色遮住线条），因此分界处只有一条线，红色只保留在单条未读圆点上。",
    kind: "fix"
  }, {
    title: "修复深色模式下左侧导航滚动条常显",
    desc: "左侧导航的默认隐藏规则只写了一个类名，它与全站 `.dark *` 规则的特异性相同（都是 0-1-0）且位置在前，因此在深色模式下被覆盖成白色 26%，滚动条一直可见；加上 `.dark` 前缀后恢复为默认透明，只有悬停与聚焦时才显示。已用无头 Chrome 加载真实样式表验证修复前后的计算值，并在 AGENTS.md 中记录这条约定。",
    kind: "fix"
  }]
};
