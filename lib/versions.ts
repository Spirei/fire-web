...
export const V0_1_25_ENTRY: VersionEntry = {
  ...
  changes: [{ title: "双向版本时光机", desc: "简化版工具栏与完整版页头新增版本穿越入口，点击即以用户提供的哆啦A梦时光机原图衔接页面切换，以 WebGL 对原图做柔和纵深与局部视差，保护人物区域，无额外流光或闪光，不支持 WebGL 时回退原图；支持深浅主题、手机小屏、减少动态效果，通过预取和应用内路由消除整页重载；隔离简化版样式并恢复返回后的窗口交互，离开前等待账本同步，失败时保留当前页面。", kind: "feature" }, { title: "版本穿越入口布局", desc: "简化版右上角直接显示无外框细线纸飞机，单击立即穿越；完整版入口保留在头像菜单，配合「去另一面」及目标版本说明。", kind: "fix" }, { title: "证券市场色块全站统一", desc: "为持仓、自选、资产分析排行、全球榜单、个股详情、公司资料、相关 ETF、交易弹窗、名人持仓、素材库和 K 线对比中的证券名称/代码组合补齐统一市场色块；美股、港股分别显示 US、HK，A 股按代码显示 SH 或 SZ，保留已正确实现的位置，并排除货币、文件、订单号等非证券代码。", kind: "feature" }, { title: "卖出回款正确计入总资产", desc: "修复历史买入缺少期初入金时，负现金净额会吞掉后来真实卖出回款的问题。现金余额改为按币种和发生时间逐笔计算，并在每一步执行零余额下限；SPCH.AM 卖出 266 股的 2,779.70 美元现会完整进入可用现金与总资产，同时买卖仍只在现金和持仓之间转换，不重复计入投入或收益。", kind: "fix" }]
};
