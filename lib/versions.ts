...
export const V0_1_27_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "段永平长回复里会露出 `&#34;`、`**加粗**`、行首 `*`；引用里的 @用户名没有高亮；Gemini 长文把一屏撑满；8 月 31 日 13:24 泡泡玛特配图被存成雪球 `!thumb.jpg`（60×80，约 3KB），原文是 2048×2731。修复：展示与落盘前解码 HTML 实体并去掉轻量 markdown；@提及高亮并链到雪球/Truth；正文默认 5 行、引用 4 行，超出显示「展开」；下载配图时去掉 `!thumb` / `!custom` 后缀取原图，已存的缩略图下次刷新会重拉。tsc 无错误。",
    kind: "fix"
  }, {
