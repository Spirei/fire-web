...
export const V0_1_29_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "“编辑分组”升级为完整左右分栏工作台并统一深浅色：左侧加入固定的“全部”，市场分组与自建分组合并为一条可自由拖拽排序的列表，数量统一右对齐，操作图标仅在悬停时覆盖显示，并把侧栏收窄到仍可完整显示最长名称的 240px；分组图标与行情板统一复用同一解析结果。新增与重命名改为一致的居中弹窗和卡面库风格胶囊按钮，新增入口紧跟在分组列表末尾。右侧补齐 min-height 约束并形成独立滚动区，73 条等长列表可从固定表头下滚动到底，切换长短列表不再导致窗口跳动；修复暗色模式下半透明浅色背景把侧栏和弹窗染成灰白块的问题。详情顶部改为全市场股票联想，选中后直接加入当前自定义分组，明细支持全选、跨分组移动、多选移出、上移/下移/图钉置顶/删除及服务端持久化排序；置顶支持多只股票，置顶区连续排列；四个操作默认隐藏、悬停或键盘聚焦行时显示；去掉文字徽标、左侧强调线和整行灰底；四个图标（含已置顶图钉）离开行后全部隐藏，划过时已置顶图钉以实心圆底区别于普通图钉；明细行增至约 56px，刷新后保留置顶状态；市场列由重复文字改为与分组栏一致的 24px 圆形市场图标，并保留悬停名称；勾选框改为 17px 中性色圆角控件，选中使用深灰底与白色对勾，深浅色不再出现系统蓝色；批量移动点击后立即更新当前列表，失败才回滚；全部和市场分组恢复上移、下移、置顶与删除操作：排序按全局或当前市场持久化，删除会从自选股中移除，导出可用态改为高对比胶囊，导入在全部及市场分组中同样可以打开并默认导入全部。行情板分组条固定显示 5 个完整分组并支持横向滑动，三横杠菜单选中其他分组后会自动滚动，并以能完整容纳当前项的最早分组为左边界，避免首枚胶囊被标题侧裁切；主题切换不再对胶囊背景色做过渡闪帧。导出改为富途兼容的制表符 TXT；新增 TXT/CSV/SEL/EBK/JSON 文件导入、列表预览和目标分组选择，并兼容 GBK 券商文件，股票默认进入全部自选并附加到所选分组；指定分组导入改为哈希索引匹配、预编译 SQL 与单事务批量写入，移除逐条全表读取造成的等待。文件选择按钮固定为 44px，底部取消与导入固定为 40px，统一恢复正常内边距与文字居中，避免被胶囊基础样式压扁。移除重复的“编辑行情板”模式，更多菜单中的编辑入口改为低强调样式。",
    kind: "feature"
  }, {
    title: "补齐线下版美股列表行情",
    desc: "修复线下版没有富途 OpenD 时，腾讯未收录的新股、冷门 ETF 与特殊代码在自选列表只显示走势图，现价、涨跌幅、最高、最低和市值却全部为横线的问题。批量行情现在会对腾讯漏项逐只使用 Yahoo 常规行情补齐，并在从线上带回“仅富途”配置但本机 OpenD 整批为空时自动启用公开源。浏览器实测 UFO、ECHO、ASTS、VSAT 六列行情完整，ECHO 列表现价与详情页一致。tsc、build 与 diff 检查通过。",
    kind: "fix"
  }, {
    title: "按富途逻辑重构自选分组入口",
    ...
    title: "优化自选股更多分组交互",
    desc: "“更多”改为不带数量的固定聚合按钮，点击后从按钮下方弹出其余分组，常用分组不再被替换，整排布局保持稳定。修复分组横向滚动容器裁掉弹层的问题，展开时解除裁剪并提高控制栏层级。弹层显示分组图标、名称、数量及当前选中标记；选中后按钮显示当前分组，点击外部自动关闭。",
    kind: "feature"
  }, {
    title: "彻底修复自选股刷新分组闪失",
    ...
    title: "移除个股财务页财报附件功能",
    desc: "删除财务页财报模块中的财报附件入口、上传归档界面、请求与本地缓存逻辑，以及仅服务于该界面的样式；摘要和收入明细保留，独立附件管理功能不受影响。",
    kind: "fix"
  }, {
    title: "卡面库 NEW 角标美化",
    ...
    desc: "右下角操作保持 32px 圆形与 44px 点击热区。全部卡面中已加入的默认显示勾，未加入的悬停显示加号；我的卡默认隐藏，悬停显示减号用于移除。按钮悬停即时提示加入我的卡／移出我的卡，触屏常显操作入口，保留详情中的加入和移除功能。",
    kind: "fix"
  }, {
    title: "卡面操作图标与提示统一中性色",
    desc: "勾号、加号和减号统一中性灰，操作提示改为 22px 高的浅灰小标签，简化为加入／移出，保留完整无障碍名称、即时提示和既有显隐逻辑；深色模式同步适配。",
    kind: "fix"
  }, {
    ...
    desc: "指数迷你走势图为昨收基准线增加上下留白，避免贴住画布边缘；涨跌色带改为只填充到基准线，虚线进一步变细变淡，走势图左右内收并完整显示末端圆点。",
    kind: "fix"
  }, {
    title: "顶部指数迷你走势图按像素抽样",
    desc: "顶部走势图按 56px 显示宽度将数百个分钟点压缩到最多 56 个代表点，每个区间保留局部最高点和最低点，并固定保留首尾点，避免等距抽样漏掉短暂尖峰；完整分钟序列继续用于行情推进判断和收盘后富途对账。",
    kind: "fix"
  }, {
    title: "全站列表趋势图统一微牛样式",
    desc: "公开首页预览与后台自选行情板共用昨收虚线火花图：去除面积填充和末端圆点，按昨收决定涨跌颜色，并以区间高低点抽样保留短暂尖峰。分时源暂时不可用时继续显示浏览器或服务端保存的最后一次成功走势，并用灰色明确标记；从未成功获取时显示灰色虚线占位。顶部指数迷你走势图保持不变。",
    kind: "fix"
  }, {
    title: "自选行情板序号统一正文颜色",
    ...
    title: "修复素材库快速操作请求竞态",
    desc: "补充请求顺序保护，快速切换分类、市场或页码时只允许最后一次请求更新界面，避免较慢的旧响应覆盖当前页面而造成卡住或错页观感；服务端分页后的序号按全局位置连续计算；搜索时立即回到第 1 页，删除末页最后一条后自动收敛到有效页码，接口失败时保留现有内容并明确提示。",
    kind: "fix"
  }, {
    ...
    desc: "确认地球图标主要出现在首屏较重的我的持仓和资产分析：浏览器会越过轻量 favicon.ico，继续等待约 492KB 的后台上传原图。现在服务端首帧只声明多尺寸静态网站图标，后台自定义图标在页面就绪且图片完整加载后再无缝替换；设置中更换图标仍会即时生效，加载失败则始终保留网站图标。",
    kind: "fix"
  }, {
    title: "消除设置页刷新时的滚动条闪现",
    desc: "设置页恢复 URL 子分类前，真实内容由仅隐藏可见性改为完全退出布局，避免其内部滚动容器透过骨架屏提前绘制右侧滚动条；状态就绪后再正常显示对应设置内容，不影响页面滚动。",
    kind: "fix"
  }, {
    title: "修复我的持仓刷新与切页闪变",
    desc: "查明 18,611 是数据库旧价格计算的持仓市值，57,515 是恢复实时行情后仍未计现金的中间值，8,020 则是首屏提前注入现金后暴露出的仅现金中间值，67,056 才是完整净资产。现在资金与卡片现金余额随服务端首屏注入，最后一次完整行情在绘制前恢复，并以完整估值状态控制金额展示；快照就绪前保留页面结构但不展示不完整数字。总资产市场状态与持仓列配置统一首帧并在绘制前恢复，消除水合重建；显示货币与市场盈亏不再短暂消失，原“各市场盈利”同步更名为“市场盈亏”。",
    kind: "fix"
  }, {
    title: "加快股票搜索联想",
    desc: "股票名称和代码优先查询本地素材库，常用标的不再等待外部接口；股票已命中时不再串行等待 CoinGecko。前端搜索防抖由 260ms 缩短为 140ms，并取消连续输入产生的过期请求，最近搜索结果缓存 60 秒。保留比特币、以太坊等加密货币联想。",
    kind: "fix"
  }, {
    title: "收敛加密货币搜索结果",
    ...
    desc: "股票搜索联想新增本地股票图标；相关 ETF 复用对应正股图标，并在图标右下角用紧凑的 2x、反向、收益标记区分产品类型。结果出现后按需批量补齐图标，不拉取完整素材库，正股与 ETF 的名称、代码和价格列保持对齐。",
    kind: "fix"
  }, {
    title: "将相关 ETF 角标调整为圆形",
    desc: "相关 ETF 图标右下角统一改为 19px 固定圆形角标，2x 保留原文，反向与收益分别缩写为“反”“收”，悬停仍显示完整含义；角标向下错开并增强边缘分隔，避免被文字撑成长方形或遮住主图标。",
    kind: "fix"
  }, {
    title: "统一全站相关 ETF 圆形角标",
    desc: "搜索联想、自选股、我的持仓、个股详情头部与相关 ETF 列表统一使用同一个关系角标组件，并从全站 ETF 关系表识别 2x、反向、收益与做多产品；全部采用 19px 圆标和右下下移位置，完整类型保留在悬停说明中。",
    kind: "fix"
  }, {
    title: "修复详情页相关 ETF 图标白底",
    desc: "股票详情的相关 ETF 列表统一复用对应正股的圆形图标，不再优先显示带白色方底的 ETF 独立素材；ETF 详情反向展示正股时仍使用该正股自身图标，圆形角标与搜索和列表保持一致。",
    kind: "fix"
  }, {
    title: "持久保存股息并增加数据源回退",
    desc: "股票详情与资产分析共用 SQLite 股息快照：已获取的历史派息永久合并保留，近期及未来记录每日更新，纯历史记录每周检查；数据源暂时失败时继续展示最后一次有效结果，不再误显示 0 期。线上启用富途时优先读取富途，本地开发态直接使用东方财富 A 股与 Yahoo 美港股公开数据，富途不可用时也会自动回退公开源；旧版误写的空缓存自动失效并重新获取。",
    kind: "fix"
  }, {
    title: "相关 ETF 行情优先显示上次缓存",
    desc: "股票详情的相关 ETF 页签保存每只产品最后一次成功行情，刷新或重新进入时在首屏绘制前恢复价格、涨跌幅和市值，同时后台获取最新行情；本次请求缺少个别标的或整体失败时保留旧值，不再清空列表并长时间显示“正在更新行情”。",
    kind: "fix"
  }, {
    title: "补齐资产分析全链路 ETF 角标",
    desc: "资产分析的持仓分布、持仓盈亏排行、订单列表与订单详情、收益日历明细，以及资产盈亏分析的股票盈亏明细，全部接入统一 ETF 关系角标；2 倍做多、反向与收益产品均按同一关系表识别，图片缺失使用字母占位时也保留角标。",
    kind: "fix"
  }, {
    title: "修复个股详情刷新闪页并缓存概览",
    desc: "概览、ETF、股息、财务与公司标签刷新时直接渲染当前个股详情，不再先闪现自选股首页；概览行情和走势图优先展示最后一次成功快照，再于后台静默更新，失败时保留旧值。全天改为直接打开完整盘前、盘中、盘后分时 K 线，夜盘等单独移入时段菜单。",
    kind: "fix"
  }, {
    title: "精简详情说明并优化 K 线周期菜单",
    desc: "删除 ETF 与股息标题下方重复说明，并复核全站同类文案，仅保留包含数据口径、范围或操作风险的信息；修复年 K 与分钟周期按钮挤压错位，增加独立分隔和横向溢出保护，周期菜单改为更紧凑的分钟、小时、长周期分区布局。",
    kind: "fix"
  }, {
    title: "统一 K 线周期控件视觉",
    desc: "移除周期按钮突兀的黄色边框、底色与箭头，统一为相邻控件使用的中性灰色交互状态；分钟、小时和长周期分别使用圆角虚线边框包裹，增强分组层次；弹层从周期按钮左缘向右展开并限制最大宽度，不再越过 K 线卡片左边界。",
    kind: "fix"
  }, {
    title: "完善 K 线工具菜单收起与指标样式",
    desc: "前复权、比较和指标菜单点击工具栏外区域后自动收起，打开其中一个菜单时同步关闭其他菜单，避免浮层残留或叠加；指标设置中的 MA 标题与复选框改为统一的中性灰白样式。",
    kind: "fix"
  }, {
    title: "恢复全部 K 线图形设置",
    desc: "修正全天与 5 日周期错误隐藏高级图形的条件，面积图、折线图、带标记线、阶梯线、HLC 区域、基准线、实心 K 线、空心 K 线和 OHLC 在所有周期始终可选，并继续保留用户的常用排序和当前样式。",
    kind: "fix"
  }, {
    title: "减轻全天选中状态",
    ...
    desc: "为 5 日、日 K、周 K、月 K 与年 K 分组补充两侧安全留白，切换后自动将当前周期完整移入可视区，避免选中背景或文字被横向容器裁切；选中态改为在同组按钮间平滑滑动。横向拖动会完整捕获并释放指针，松手后不再粘连移动。",
    kind: "fix"
  }, {
    title: "聚合复权与指标设置",
    desc: "移除工具栏独立的前复权按钮，将前复权、不复权和后复权作为紧凑分段选项并入指标面板；聚合入口继续使用原指标图标，面板标题与分区标题统一左对齐，下方均线与显示设置保持原有功能。",
    kind: "fix"
  }, {
    title: "解耦全天视图与分钟周期",
    desc: "全天作为独立视图，不再读取、改写或高亮分钟与小时周期，并默认不叠加 MA；面积图和折线图使用完整原始分时数据，蜡烛类图形仅在内部按屏幕可见密度聚合，图形类型仍可自由切换。修复周期按钮因过早捕获指针而无法点击、关闭全部指标后持久化设置被默认值覆盖，以及指标切换时反复销毁图表实例导致主价格图消失的问题。",
    kind: "fix"
  }, {
    title: "精简 K 线跟随框并自动避让",
    desc: "参考富途压缩全天分时跟随框的信息密度，展示价格、估算均价、涨跌、成交量和成交额；估算均价按可用分钟成交量加权，无有效成交量时显示横线，避免把当前价误当均价。5 日及日 K、周 K、月 K、年 K 等大周期继续完整展示开高低收与成交信息；所有周期的触点在图表左半区时跟随框固定右上角，越过中线后切换到左上角，并在主价格折线或 K 线柱收盘位置显示跟随圆点。",
    kind: "fix"
  }, {
    title: "恢复 K 线周期持久化",
    desc: "刷新或切换股票时恢复最后选择的全天、分时、5 日、日 K、周 K、月 K、季 K、年 K或今年以来视图，同时保存全天与自定义分钟的区别；首次恢复完成前禁止默认状态写回本地缓存，避免已保存周期被全天覆盖。图形样式的服务端与浏览器首帧统一使用面积图占位，挂载后再恢复偏好，避免本地偏好造成 hydration 错误。",
    kind: "fix"
  }, {
    title: "优化 K 线跟随点层级",
    desc: "完整恢复蓝色实心圆点、白色描边与顶层显示；K 线图仅将圆点定位到开盘价与收盘价之间的实体柱中点，避开上下影线。折线和面积图仍精确落在价格线上。图形样式、复权和指标恢复完成前禁止默认面积图写回设置，实心 K 线等选择刷新后保持不变。",
    kind: "fix"
  }, {
    title: "股息记录改为持久快照优先",
