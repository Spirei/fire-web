...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "对话右侧的轮次短线按「从上到下由旧到新」，但打开对话时高亮却固定停在第一轮（最上面），看起来像最新一轮在上面。现在打开对话或新增消息后高亮自动落到最新一轮，也就是最下面那条；最新一轮的短线还加了像音符一样的循环跳动动画（尊重系统「减少动态效果」）。轮次很多时导航高度收在面板内、从顶部裁掉最早的几条，保证最新一轮始终可见。",
    kind: "fix"
  }]
};
