...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "持仓盈亏桑基图按宽度自适应，手机端不再丢标签",
    desc: "手机端「我的持仓」的持仓盈亏图此前只有宽 / 窄两套写死的排版：左右各留 82px、标签固定两行 9px 小字 —— 366px 的画布上两端留白吃掉 45% 宽度，中间只剩约 190px 画画，流带被挤成两道竖帘；更糟的是小持仓的标签与相邻标签重叠后会被 ECharts 的 hideOverlap 整个隐藏（实测 17 只持仓里「标普500ETF」「小鹏汽车」直接不见），看起来就是「没有自适配、被拉宽」。现在新增 lib/sankeyLayout.ts 按可用宽度连续计算排版：标签列宽按 24% 收缩（78–118px）、边距 = 标签宽 + 间距、字号 10 / 9、节点间距加大到 18（必须大于标签行高），手机端标签改成单行「名称 + 金额」，把省下的高度还给流带；画布高度改为随持仓数增长（52px / 只 + 80，限 360–820px）；宽度变化超过 24px 才重算布局，避免键盘弹出等抖动反复 setOption。宽屏（≥720px）保留两行标签，并把节点间距从 18 提到 32（两行标签需要 ≥30），此前被藏掉的 5 个小持仓标签同样回来了。用 ECharts SSR 实测 360 / 366 / 390 / 430 / 768 / 1024 / 1180 七种宽度、17 只持仓：改前 366px 丢 2 个标签、1180px 丢 5 个，改后全部可见；回归用例覆盖「手机留白占比、流带宽度、节点间距与行高的关系、标签宽下限、画布高度上下限」以及组件确实接上了这两个函数。",
    kind: "fix"
  }, {
    title: "持仓盈亏图观感重做：渐变连线、分色柱体与随主题的调色板",
    desc: "持仓盈亏图此前是「一坨淡粉 + 一坨淡绿 + 一根紫色竖线」：连线是单色半透明色块、柱体是细色条、中间那根又是与红绿无关的紫色，配上两行小字，在手机屏幕上显得糊且廉价。这一轮整体重做观感：连线改成沿「源 → 目标」的显式渐变（红 → 中性 → 绿），大持仓那条宽带不再是一整块颜色；柱体改成带纵向渐变的红 / 绿圆柱，加粗到 11px、圆角 6px；中间的汇总柱由紫色改为中性深灰渐变，作为结构件而不是第三根彩色棒；连线透明度降到 0.34、曲率收到 0.45，让文字与柱体站得住。配色按深浅色分成两套调色板（深色底上不再发白、脊柱不再亮得像荧光棒）。同时把图表配置抽成 lib/sankeyModel.ts 纯函数，本地就能用 ECharts SSR 渲染出与线上一致的效果逐版对比 —— 过程中因此发现并规避一个渲染陷阱：ECharts 的 lineStyle.color: 'gradient' 是 canvas 专用写法，SVG 渲染器会写成无效的 fill=\"gradient\" 并附带一层裁剪把两侧标签整片切掉，因此改用显式渐变对象（两种渲染器都认），回归用例专门锁住这一点与深浅强调色板。",
    kind: "feature"
  }, {
    title: "持仓盈亏图手机端不再被压扁",
    desc: "配色与渐变保持上一版不变，这一版只修比例：每侧最多 6 只持仓，而画布高度此前按「52px / 只 + 80」算（6 只 → 392px），在 390px 宽的手机上几乎是个正方形 —— 六行持仓连同那条最大的柱子挤在一起，曲线也被压得很平。现在手机端每只持仓的高度预算提到 76px（6 只 → 552px，390 宽下宽高比约 1:1.4），节点间距从 18 提到 24，让台积电 / 腾讯控股 / 伯克希尔B 这类小持仓不再挤成一堆；桌面端宽度足够，由 CSS 把高度收回 min(…, 460px)，避免被拉得过高。回归用例补上「6 行持仓时手机高度 ≥520px、高度不低于 390 宽的 1.3 倍」与桌面收口的 CSS 断言；ECharts SSR 实测 390×552 与 1180×460，12 个标签零丢失。",
    kind: "fix"
  }, {
    title: "持仓盈亏图手机端柱体加宽为胶囊条",
    desc: "接着手机端比例那版，这一版把柱体本身做粗：手机端柱宽从 11px 提到 16px、圆角 6 → 8（正好是胶囊端头），柱子从「细色条」变成有分量的锚点。档位是渲出来比过的：15–19px 最合适，24px 时柱子开始像色块、中间那根脊柱也太重；而每加宽 1px 都在吃中间流带的横向长度（11 → 16 会少掉约 10%，153px → 138px），所以停在 16。桌面端宽度充足，维持 14px / 圆角 5 不变。ECharts SSR 实测 390×552 与 1180×460，12 个标签零丢失；回归用例补上「手机端柱宽 ≥15px」「加粗后仍要留出流带的横向长度」「桌面柱宽保持 14px」。",
    kind: "feature"
  }]
};
