...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "删除确认弹窗的按钮补回底色与尺寸",
    desc: "弹窗底部按钮此前只挂了 .btn（纯布局）和一个并不存在的 btn-secondary 类：取消按钮因此没有任何底色与文字色，深色模式下几乎看不见；删除类确认按钮也缺内边距，看起来像一行红字而不是按钮。现在取消按钮统一为全站中性样式（白底 + 浅灰边框 + 深色文字，深色模式深灰底 + 白字，悬停品牌浅灰），danger 确认按钮改为实心红底白字并补禁用态，底部间距按弹窗规范改为 10px。顺带把源码里用到的 btn-* 类与 globals.css 的定义做了一次比对，确认不再有其它「用了但没定义」的按钮类。",
    kind: "fix"
  }]
};
