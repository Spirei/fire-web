...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "除独立智能助手工作台外，主站页面统一增加当前页面顶栏；移除重复的 Fire / 前缀，持仓页显示我的持仓，页面内部资产卡片沿用账户资产，资产分析、FIRE 与全球经济显示各自页面名及对应四色门状态。顶栏与内容归入同一个圆角工作区外壳，内边距按可用宽度自适应，不修改持仓表格、资产分析分栏、设置窗口等页面内部列宽，并完成深浅色适配；矮窗口同步增加手势与侧栏分隔线的间距。",
    kind: "feature"
  }]
};
