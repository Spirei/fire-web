...
export const V0_1_28_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "券商↔卡转账联动记账（不再重复计算总现金）+ 卡面库总览拆「余额 / 额度 / 其他」",
    desc: "修掉两处口径问题。1) **重复计算**：卡包「存钱 / 取钱」记的那笔钱，如果本来就在券商账户里（从券商转到卡上、或从卡上转回券商），券商账本不会自动少一笔 —— 卡余额被算进现金、券商那边的钱也还在，总现金直接翻倍。现在卡包的「存钱 / 取钱」弹窗加了一个**苹果风格开关**「这笔钱是从券商账户转来的 / 转回券商账户」（只在资金系统支持的 7 个币种下出现）：打开后 `addCardBalanceEntry` 在**同一个数据库事务**里再写一笔**方向相反**的资金流水（卡里存入 = 券商转出、卡里取出 = 券商转入），两边永远对称；流水 id 用 `card-link-{卡包流水 id}`，删卡包那笔时联动流水一起删，资金记录里标为「自动」且不可单独删（显示备注说明是哪张卡）。实测：CNY 卡存 100 后券商账本 -100、卡余额 +100，总额仍是 10,719.50（改动前会变成 10,819.50）；取回 100 反向对称；两笔都删掉后完全回到初始状态、无残留流水。2) **总览口径**：卡面库「我的卡」总览原来只有一个「额度合计」，而「金额」在借记卡上是余额、在信用卡上是额度，混在一起既没意义、又和资产分析的「银行卡现金」对不上。现在拆成**余额合计（借记卡 / 预付卡）· 额度合计（信用卡）· 其他合计**三档，各自换算成显示货币、各自标注缺汇率的币种，鼠标悬停说明「余额合计就是计入可用现金的那一份、额度是可透支上限不计入」。3) 顺带修 `createFundTransaction` 的返回值：原来用「最近一条」回读，补录旧日期的流水时返回的是别的记录（接口 POST 响应可能给错数据），改为按 id 回读。验证：tsc 无错误、冒烟 114/114 全 PASS。",
    kind: "fix"
  }, {
    title: "资金系统币种扩到全币种：英镑 / 台币 / 澳门元 / 澳元 / 加元卡也能联动券商账户",
    desc: "上一版卡包的「券商账户」联动只在资金系统的 7 个币种下出现，台币 / 澳门元 / 英镑这类卡没法用。这次把资金系统的币种扩到**所有能折算的币种**。1) 新增 `lib/fundCurrencies.ts`（客户端 / 服务端共用，不引入服务端依赖）：`FUND_CURRENCIES` 共 14 个 —— 7 个主币种 + 卡面素材里实际会出现的台币 / 澳门元 / 英镑 / 澳元 / 加元 + 有汇率但暂无卡面的印度卢比 / 巴西雷亚尔；口径是**能折算才收**（每个币种都在 `FALLBACK_RATES` 里有汇率兜底），卢布 / 坚戈不进清单 —— 进来就会在跨币种汇总时按 1:1 当成美元误算。同时提供 label / 符号 / 国旗二字码、`isFundCurrency()`、`emptyFundBalances()`、`fundCurrencyOptions()`。2) 数据库：`fund_transactions` 的 currency 约束从**硬编码币种列表**改成通用的「三字母大写」约束（历史是 3 币种 → 7 币种 → 现在清单还会继续长，硬编码意味着每加一个币种就要重建一次表）；旧库检测到硬编码 IN 列表即无损重建（连索引一起重建），以后再加币种不必再动表。3) 后端：`fundBalances()` / `fundSummaries()` 按新清单初始化，并给清单外的历史脏币种加 `|| 0` 兜底（否则会让整个币种余额变成 NaN）；`/api/v1/funds` 的校验改走 `isFundCurrency()`（顺带把币种统一转大写再校验）；数据备份导入导出的币种白名单同步。4) 前端：`CurrencySelect` 新增 `options` 参数（默认仍是全站 7 个展示币种，顺手删掉从未被任何调用方使用的 `allowedCodes`），资金系统面板与「记一笔」弹窗改用 14 币种清单（带国旗）；资金余额相关类型从 `CurrencyCode`（展示币种）换成 `FundCurrency`，把「展示币种」与「资金币种」这两个一直混用的概念分开。5) 卡包：存钱 / 取钱的券商账户开关改用 `isFundCurrency()` 判定。验证：tsc 无错误；实测 GBP 资金流水写入成功（说明 DB 约束已放宽）、RUB 被正确拒绝（「资金记录参数无效」）；再把一张卡临时改成 GBP 做联动存钱，返回 `brokerLinked: true` 且生成的 `card-link` 流水币种为 GBP，随后删流水 + 还原币种，数据完全回到初始状态（卡余额 10,719.50 CNY、无残留流水）。",
    kind: "feature"
  }, {
    title: "大陆借记卡不显示安全码（卡背 / 卡片信息 / 编辑弹窗三处一致）",
    desc: "现象：卡包详情里，中国大陆的借记卡卡背同样画着 CVV 白方块（没填就显示「———」）、卡片信息里也列着「安全码」一行 —— 而大陆借记卡背面本来就没有安全码（线上支付走密码 / 短信验证，卡号 + 有效期也刷不了）。修复：新增 `hasSecurityCode(card)` 规则（**中国内地 + 借记卡** → 不显示；其他地区 / 卡种是否带安全码暂无结论，一律保持原样，规则集中在一处、以后再扩只改这一行），三处保持一致 —— ① 卡背不再画 CVV 方块（签名栏顺势拉满）；②「卡片信息」不列「安全码」这一行；③ 编辑弹窗不出现安全码输入框（有效期独占一行）。同时「隐藏卡号 / 安全码」文案按是否有安全码自适应成「隐藏卡号」，底部说明补一句原因；保存时这类卡不再提交 `cvv` 字段（接口按「只覆盖传入字段」处理，历史残留值既不会被写回、也不会被这次保存顺手改掉）。验证：tsc 无错误、npm run build 通过、冒烟 114/114 全 PASS。",
    kind: "fix"
  }, {
    ...
    title: "素材库新增「卡片」分类 + 卡面统一到素材库 + 卡包详情换卡面与眼睛图标",
    desc: "按反馈一次补齐三件事。1) **素材库新增「卡片」分类**：清单里的 436 张卡面全部登记成 `assets.type = \u0022card\u0022`（id 约定 `card:{卡面文件}`，大小写敏感、不走 upsertAsset 的大写化，否则含 ASCII 文件名的路径会被改坏），进类目时幂等补齐、**已换过图的卡绝不覆盖回去**；列表按地区 + 卡名排序、支持搜索、分页，点缩略图即可换图，换过图的卡多一个「恢复原图」；列表 / **网格**（卡面墙，一屏看更多图、挑图方便）两种展示方式可切换；展示方式属于「此刻看什么」的视图状态，按约定走 **URL**（`?view=grid`，list 为默认值不写进地址栏），刷新 / 前进后退 / 分享都保持，切换时回到第 1 页；分页改成按「**排**」算而不是固定 10 条 —— 列表 6 排、网格 3 排，列数跟随 sm / lg / xl 断点实时重算（列表 3/2/1 列、网格 5/4/3/2 列），所以任何宽度下都正好是 6 排 / 3 排（436 张：列表 18 条/页、网格 15 条/页，页数从 44 页降到 25 / 30 页）；换图入口不要文字 —— 悬停只留一层很淡的暗色蒙层，上传中显示转圈，原来那行「换图」文字去掉；切换图标也重画过 —— 原来的四宫格用 1.8 描边 + 2 单位间隙，实际渲染出来四格糊成一坨黑块、列表则是三条毫无层次的横线，现在改成 1.5 描边、方格圆角 + 2.8 单位间隙（四格分明），列表换成「圆点 + 三行」（一眼认出是列表），未选中态也从 `text-muted` 收到 `text-faint`，选中态不那么抢眼。2) **卡面统一到素材库**：此前卡面有两套来源（清单原图 + 卡面上传的个人覆盖），会出现「在素材库换了图、卡面库没变」这种两处各管一份的问题；现在卡面库网格 / 弹窗大图 / 卡包堆叠 / 卡背正面统一读 `/api/cards` 新下发的 `covers`（卡面文件 → 当前图片地址，来自素材库那条卡片素材），卡面库弹窗里的「上传卡面」也改成写这条素材 —— 一个地方换图，处处同步（`card_details.image` 那套个人覆盖随之废弃并停止写入）。3) **卡包详情也能换卡面 + 眼睛图标**：卡包详情的「卡片信息」头部新增「换卡面」（与素材库·卡片同一条，改完同步回卡面库）；卡号 / 安全码的显示切换从原来那行文字按钮改成卡片详情**右下角的眼睛图标**（睁眼 = 已显示、闭眼加斜杠 = 已隐藏，按下时高亮），文案位置只留一个「已显示 / 已隐藏」状态提示，避免和眼睛重复。验证：tsc 无错误；实测 `GET /api/assets?type=card` 登记出 436 条（地区分布 中国内地 251 / 中国台湾 50 / 英国 29 / 中国香港 28 / 美国 22）、`/api/cards` 的 `covers` 同步 436 条；把某张卡的素材 url 指向另一张卡后卡面库 payload 立即跟着变、恢复原图后回到清单地址。",
    kind: "feature"
  }, {
    title: "修复换卡面会删掉卡面清单原图（数据丢失）",
    desc: "现象：在素材库「卡片」里把某张卡换成自己的照片后，那张卡的**清单原图文件被删掉了** —— 再点「恢复原图」会把卡面指向一个不存在的文件，等于换一次图就永久弄丢原图。根因：`upsertAsset` 在 url 变化时会调 `removeFileIfUnused(旧 url)`，而 `urlReferenced()` 只认 assets / 网站设置 / 用户头像 / 名人头像这几处引用 —— 卡面清单原图（`public/uploads/cards/**`）是卡面库直接从 `manifest.json` 读的文件，**数据库里没有任何字段引用它**，于是被判成孤立文件删掉。修复两处：① `urlReferenced()` 对 `/uploads/cards/` 下的文件直接返回 true（清单原图永久保留 —— 它不是用户上传物，整个目录由抓取脚本管理）；② `cleanupOrphanFiles()` 扫描时整棵跳过 `public/uploads/cards/`（否则一次孤立文件清理会把 436 张原图全清掉）。验证：复现「换图 → 恢复原图」两步，修复前原图文件消失、修复后原图仍在且 `/uploads/cards/...` 返回 200；测试中被误删的那张原图已从 git 恢复。",
    kind: "fix"
  }]
};
