...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "相关 ETF 角标改为名称旁的内联标签",
    desc: "挂在股票 logo 右下角的那颗 2× 角标换掉了：主流做法（雪球 / 富途 / 微牛）是把「2×做多」「反向」这类属性做成名称旁边的标签，而不是往 logo 上盖角标 —— 角标压住图标、小字挤在圆里、颜色一重就显得压抑（深灰圆点与浅色胶囊两版都被否掉）。现在 components/EtfDoubleBadge.tsx 与 MarketCodeBadge 同规格：18px 高、10px 加粗、4px 圆角的内联小标签，放在名称下方的代码行里（市场徽标之后），杠杆倍数用乘号写作 2×；配色按类型分四档浅底深字（2× 琥珀、做多红、反向绿、收益策略蓝），深色模式各有一档。8 个页面共 10 处调用点全部从图标容器挪进信息行，股票详情页「相关 ETF」列表原本与名称旁的灰色「2X 做多」标签重复，直接把角标去掉。回归用例锁住「组件不再绝对定位、显示 2×、四档配色齐全、各页面都不再把标记挂在图标容器里」。",
    kind: "feature"
  }]
};
