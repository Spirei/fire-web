...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "新增全站上下文账户助手",
    desc: "全站右下角新增账户助手：桌面使用不遮挡主内容的右侧浮动面板，手机使用底部面板；自动识别账户资产、资产分析、资产总盈亏、自选股、FIRE、财报日历、卡面库和素材库等当前页面，并提供对应快捷问题。服务端复用设置中的大模型提供商、API 地址、模型与密钥，只向模型提供当前用户的受控持仓摘要，密钥不会下发浏览器，模型不能直接读写数据库；未配置模型时仍可在本地完成持仓概览和基础数据诊断。回答明确区分事实、限制与下一步，写操作留待后续加入预览、确认、撤销和审计后开放。",
    kind: "feature"
  }, {
    title: "账户助手刷新后恢复上次对话",
    desc: "账户助手按用户保存最近 30 条消息，刷新页面或切换功能后重新打开仍可继续上次对话；不同登录用户使用独立缓存，避免共用浏览器时串出对话。标题栏新增“开始新对话”按钮，可随时清空历史重新提问；损坏或不可用的缓存会自动降级，不影响当前会话。",
    kind: "fix"
  }]
};
