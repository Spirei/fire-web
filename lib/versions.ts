...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "账户总览银行卡现金跨市场关联",
    desc: "修复资产分析选择美股、港股或 A 股后，银行卡现金只读取该市场结算币种、导致农行 5000 元等其他币种借记卡余额显示为 0 的问题。银行卡现金现在作为账户级资产始终按当前展示货币折算，并同步计入筛选视图的可用现金与净资产；计算时扣除市场资金余额中已包含的同币种卡余额，避免重复。桌面端账户总览的 7 项指标由六列改为七列，保持单排显示。货币国旗的服务端映射同时移至通用模块，避免开发环境服务端布局读取客户端常量引发运行时错误。",
    kind: "fix"
  }]
};
