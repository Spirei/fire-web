...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "账户总览资金项按视图显示",
    desc: "账户总览的资金口径按视图拆清：银行卡现金与冻结现金只在“全部”下显示，且银行卡余额按当前展示货币折算后计入全部视图的可用现金与净资产；美股、港股、A 股只保留净资产、当日盈亏、持仓市值、浮动盈亏、可用现金 5 项，并继续使用各市场自己的资金余额。响应式断点从 xl 前移到实际面板可用的 lg，桌面下“全部”7 项、单市场 5 项都保持一排。货币国旗的服务端映射同时移至通用模块，避免开发环境服务端布局读取客户端常量引发运行时错误。",
    kind: "fix"
  }]
};
