...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "自选分组筛选刷新后保持",
    desc: "修复我的行情板选择分组后只更新组件内存、刷新即返回全部的问题。顶部胶囊、更多分组菜单和分组面板现在统一把所选分组 id 写入 URL 的 filter 参数；进入股票详情会保留当前分组，返回列表继续显示原筛选，浏览器前进后退同步恢复。旧 M:/G: 参数自动迁移为分组 id，分组删除或链接中的 id 已失效时自动清理并回到全部。",
    kind: "fix"
  }]
};
