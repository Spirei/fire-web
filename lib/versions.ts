...
export const V0_1_27_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "交易广场改成「只保留最近 30 天」（不再返回历史旧帖）",
    desc: "用户明确：动态只看近期，老帖是过去式、没有展示意义。改为在接口层过滤——只返回最近 30 天内发布的帖子（磁盘缓存不删数据，只是不再返回），并按作者 400 条兜底防止某作者刷屏；客户端读取缓存与合并时用同一个窗口收敛。实测：接口由 491 条变为 490 条、最早日期从 2026-07-19 收敛到 2026-08-21（现有数据基本都在 30 天内，所以条数变化不大；要更短只需把 TRADING_SQUARE_RECENT_DAYS 调小）。新帖照常更新。tsc 无错误、npm run build 通过、冒烟 113/113 全 PASS。",
    kind: "feature"
  }, {
