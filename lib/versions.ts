...
export const V0_1_19_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "发布状态页新增最近一年 GitHub 风格更新热力图，按每日运行次数显示绿色深浅；立即构建改为舒缓绿色呼吸圆点，手机端工具按钮右对齐；页面增加服务端管理员守卫，非管理员统一返回 404。",
    kind: "feature"
  }, {
    ...
    desc: "通过 iPhone 12 Pro 视口定位图表工具栏左侧周期组被 Flex 压缩至 41px 的问题；手机端改为周期与图表操作上下分区，K 线周期在自身区域平滑横滑，复权、比较、图形和指标按钮保持完整文字，极窄屏允许操作组自然换行。顶部行情指标改为默认隐藏，可按需显示为 2×2 卡片；时段、周期、复权、比较、图形和指标选择框统一使用视口内底部安全面板，解决弹层越界；移动断点按视口将 K 线高度收敛至约 220–300px，避免手机图形纵向占位过大；详情头部同步缩小返回键与图标、截断过长名称、隐藏重复代码并收紧价格排版。",
    kind: "fix"
  }]
};
