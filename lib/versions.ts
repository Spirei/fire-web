...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "正文里的股票提及此前只显示名称 + 虚实线下划线（按涨跌着绿/红），不太容易一眼认出这是股票。现在按雪球习惯渲染为蓝色链接 $名称(代码)$：原文里的「西方石油」、单独的「OXY」，以及雪球原文自带的 $西方石油(OXY)$，统一显示成 $西方石油(OXY)$，颜色取雪球蓝 #3774e0（深色模式 #5b91f0），悬停加下划线并变亮，点击仍打开原来的行情卡片与个股详情。",
    kind: "fix"
  }]
};
