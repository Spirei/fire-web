...
export const V0_1_27_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "段永平长回复里会露出 `&#34;`、`**加粗**`、行首 `*`；引用里的 @用户名没有高亮；Gemini 长文把一屏撑满；8 月 31 日 13:24 泡泡玛特配图被存成雪球 `!thumb.jpg`（60×80，约 3KB），原文是 2048×2731。修复：展示与落盘前解码 HTML 实体并去掉轻量 markdown；@提及用雪球蓝虚线高亮并链到主页（此前只用了和正文几乎同色的 brand-deep，看起来像没高亮）；正文默认 5 行、引用 4 行，超出显示「展开」；下载配图时去掉 `!thumb` / `!custom` 后缀取原图，已存的缩略图下次刷新会重拉。tsc 无错误。",
    kind: "fix"
  }, {
    title: "交易广场股票名按涨跌着色（涨红绿跌虚线）",
    desc: "贴文里的股票名虚线原先固定灰色。改为挂载时拉行情：涨（changePct≥0）红色虚线，跌绿色虚线，颜色与全站涨跌色一致；行情未返回前仍用灰色。同一代码多处出现只请求一次。tsc 无错误。",
    kind: "fix"
  }, {
