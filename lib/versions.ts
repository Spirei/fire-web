...
export const V0_1_30_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    desc: "图片支持仅本次与持久保留双模式；保留文件进入用户隔离的私有数据目录，只能经登录鉴权接口查看，删除归档同步清理记录与文件。对话可归入自建空间。新增模型调用账本，记录实际服务、模型、状态、延迟、错误和提供商返回的 Token；归档内展示调用与失败统计，价格未知时不虚报费用。",
    kind: "feature"
  }, {
    title: "模型发送内容透明化",
    desc: "输入区新增发送内容面板，在请求前明确显示将使用的第三方模型服务，以及文字、图片、当前页面摘要、账户摘要和记忆中哪些内容会被发送；同步提示第三方服务可能按自身条款处理或保留请求，提醒不要输入密码、Cookie 或 API Key。",
    kind: "security"
  }, {
    title: "智能助手补齐模型控制与流式对话",
    desc: "输入区新增自动或指定模型选择、三档数据发送范围和临时对话；兼容 OpenAI SSE 的服务逐段显示回答并可随时停止，不支持流式时继续使用完整响应与顺序回退。每条回答显示实际服务、模型及是否发生回退，并支持复制、重新回答、编辑问题后建立分支和导出当前对话。",
    kind: "feature"
  }, {
    title: "智能助手增加可控记忆与服务健康状态",
    desc: "对话归档内新增默认关闭的跨对话记忆，可查看、编辑或一键清空；仅开启后才把用户填写的记忆注入模型上下文。新增模型目录接口与进程内健康状态，正常问答和连接测试都会记录最近成功、失败、耗时与检查时间，为自动回退和故障排查提供依据。",
    kind: "feature"
  }, {
    title: "模型密钥改为服务端加密存储",
    desc: "模型服务密钥、旧版大模型密钥、雪球 Cookie 与数据库密码写入 SQLite 前统一使用 AES-256-GCM 加密，读取时仅在服务端解密；支持 FIRE_ENCRYPTION_KEY，未配置时生成权限为 0600 的持久化实例密钥。现有明文配置保持兼容，并在下次保存时自动迁移为密文。",
    kind: "security"
  }, {
    title: "智能助手输入区移除附件按钮",
    ...
    title: "账户助手更名并适配深浅色",
    desc: "全站入口、面板标题和无障碍标签统一由“账户助手”改为“智能助手”。面板、归档遮罩与抽屉、会话气泡、错误和操作卡片、输入发送区补齐深色配色，浅色模式保持原有轻盈白色视觉。",
    kind: "feature"
  }, {
    title: "账户助手打开后保留入口",
    desc: "账户助手面板打开后，页面底部原来的圆形入口继续保留在用户放置的位置，并提高到遮罩上方保证可点击；再次点击入口可直接收起面板，拖动位置继续持久化。",
    kind: "feature"
  }, {
    title: "精简账户助手标题栏",
    desc: "移除会把面板压窄并造成标题换行变形的最小化按钮，标题栏只保留对话归档、新建、置顶和关闭四个必要操作，并收紧移动端间距。归档抽屉删除重复的新建对话大按钮，统一从标题栏加号新建。标题栏按钮独立隔离拖拽事件；置顶复用设置页相同的竖直图钉 SVG，并与加号统一为相同尺寸、圆形背景和交互效果，置顶后以同色实心图钉表示状态。",
    kind: "fix"
  }, {
    title: "对话历史改为归档并长期保留",
    desc: "账户助手标题栏始终显示历史对话与新建按钮；历史抽屉改为对话归档，不再按数量截断会话，已有和后续会话全部保留，可单独删除。单个会话仍限制消息数量与大小，避免异常内容占用过多空间。",
    kind: "feature"
  }, {
    title: "修复账户助手图钉点击",
    desc: "图钉按钮增加独立的指针事件隔离和层级，点击不再被标题栏拖动处理截获；置顶与取消置顶均可正常切换，标题栏拖拽仍保持可用。",
    kind: "fix"
  }, {
    title: "收敛账户助手图钉颜色",
    ...
    desc: "关闭状态的圆形入口与打开后的账户助手面板均可直接拖动：入口拖动不会误触打开，面板通过标题栏移动且标题栏按钮仍可正常操作。两种状态分别按用户保存在浏览器，刷新和开关后恢复上次位置；视口尺寸改变或保存位置越界时自动收回可见区域，手机与桌面都可使用。标题栏新增持久化图钉，置顶后点击面板外部不会收起。",
    kind: "feature"
  }, {
    title: "修复模型图标上传并调整排序交互",
    desc: "修复选择模型图标后异步上传完成时文件输入节点已释放而触发空引用报错的问题。模型图标改为按原图透明背景、完整比例直接展示，不再额外套品牌色底板、边框或圆角。服务优先级调整移到模型服务只读列表，配置多个服务后可直接拖动卡片并立即持久化，编辑页移除服务上下移动按钮，只保留配置、模型内部顺序和删除操作。",
    kind: "fix"
  }, {
    title: "模型配置增加逐项连接测试",
    desc: "每个模型行新增独立测试入口，直接显示连接成功耗时或可理解的失败原因，保存前即可确认 API 地址、密钥与模型 ID 是否匹配。测试接口仅管理员可用，限制频率与请求大小，已保存密钥由服务端按服务 ID 读取且不会回传浏览器；自定义图标可一键恢复提供方默认图标。",
    kind: "feature"
  }, {
    title: "模型服务支持自定义图标与多级回退",
    desc: "模型服务从单项配置升级为有序服务列表：每个提供方可自定义显示名称并上传本地图标，一个服务可添加多个模型 ID；服务与模型都能调整优先级，第 1 项调用失败、超时或返回空内容时，账户助手和内容翻译会依次回退。新增服务默认作为其他 OpenAI 兼容提供方，不再把“自定义服务”当成固定品牌。密钥逐服务保存且只向浏览器返回配置状态，旧版单模型配置首次编辑时无损迁移；素材清理会保留仍被模型服务引用的图标。",
    kind: "feature"
  }, {
    title: "设置页重塑为模型服务",
    ...
    desc: "自选首次进入立即加载报价，手动刷新可获取休市快照，定时刷新遵守持久化周期，后台返回仅到期补刷；超过 100 只按 API 上限分批。手机导入弹窗可滚动至底部操作。普通用户通过归属校验的独立接口上传分组图标，公共素材权限保持不变；财报全站排序仅管理员可改，失败回滚并提示。版本列表补齐当前和上一版，本地腾讯提示不再误报富途故障。增加隔离数据库回归检查。",
    kind: "fix"
  }, {
    ...
    desc: "修复两倍做多等相关 ETF 在素材库已有发行商图标时不跟随正股的问题。统一图标层现在无条件以关系表中的正股图标覆盖 ETF 及 .AM、.N、.OQ、.PS、.K 后缀代码，客户端全量素材与服务端首屏预载使用同一规则；正股图标缺失时回退名称首字母，不再显示 ETF 发行商图标。SPCH.AM、TSLL、NVDX、ROBN、AAPU、RKLX、SMCX 等均覆盖。",
    kind: "fix"
  }, {
    title: "自选分组筛选刷新后保持",
    desc: "修复我的行情板选择分组后只更新组件内存、刷新即返回全部的问题。顶部胶囊、更多分组菜单和分组面板现在统一把所选分组写入 URL 的 filter 参数；进入股票详情会保留当前分组，返回列表继续显示原筛选，浏览器前进后退同步恢复。分组删除或链接参数失效时自动清理并回到全部。",
    kind: "fix"
  }, {
    title: "自选分组 URL 改为短标识",
    desc: "自选分组筛选地址不再暴露 wg- 开头的内部随机 id：市场分组使用小写市场码（us、cn、hk、sg、jp、kr 等），自定义分组按当前排序使用 1、2、3……；分组重排后当前地址同步更新序号。旧内部 id、M:市场和 G:名称格式不再兼容，遇到时直接清除参数并显示全部，不执行转换或跳转。",
    kind: "fix"
  }, {
    title: "全站货币国旗首屏提速",
    desc: "货币国旗继续全部以素材库为配置来源，但不再由每个 CurrencyFlag 实例触发完整 250 个国家/地区素材请求。服务端按素材库记录读取固定 7 种展示货币对应的本地文件，并以内联图片随首屏 HTML 下发，刷新第一次绘制不再依赖第二个 /uploads 请求；远程自定义素材仍预载，资金系统的扩展币种只在组件实际出现时按 ISO 代码请求。我的持仓、资产分析、资金系统、FIRE、名人持仓和卡面库统一受益，素材库替换国旗后仍全站生效。全球经济因需要完整国家目录，继续按需读取全部国旗。",
    kind: "fix"
  }, {
    title: "账户总览资金项按视图显示",
    desc: "账户总览的资金口径按视图拆清：银行卡现金与冻结现金只在“全部”下显示，且银行卡余额按当前展示货币折算后计入全部视图的可用现金与净资产；美股、港股、A 股只保留净资产、当日盈亏、持仓市值、浮动盈亏、可用现金 5 项，并继续使用各市场自己的资金余额。响应式断点从 xl 前移到实际面板可用的 lg，桌面下“全部”7 项、单市场 5 项都保持一排；指标区增加最大宽度，宽屏下不再被拉得过散。货币国旗的服务端映射同时移至通用模块，避免开发环境服务端布局读取客户端常量引发运行时错误。",
    kind: "fix"
  }, {
    title: "素材图标刷新首帧不再空白",
    ...
    desc: "将 SpaceX 两倍做多 ETF 从搜索源遗留代码 SPCH.AM 统一为实际代码 SPCH。数据库迁移保留有 604 股及 13 笔历史订单的真实持仓，删除截图导入产生的空仓重复记录，并同步订单、活动与素材代码；新增和编辑记录也统一经过美股交易所后缀规范化，文件与截图导入沿用同一规则，后续不会再次生成 SPCH.AM。",
    kind: "fix"
  }, {
    title: "卡面库首屏卡面稳定加载",
    desc: "修复卡面库刷新后图片区域先全部为空、随后集中出现造成抖动的问题。服务端按当前用户持有清单预载“我的卡”首屏最多 12 张，客户端首批 32 张立即加载且前 8 张提高优先级；卡槽始终保持银行卡标准比例，后续卡面仍继续懒加载，不会为首屏一次请求完整 433 张素材。",
    kind: "fix"
  }, {
    title: "导航图标与头像刷新不再闪空",
    desc: "左侧导航当前需要的本地素材图标改为服务端内联进首屏 HTML，头像也在服务端读取本地文件并以内联图片交给账号菜单；客户端重新确认登录态时保留同一首屏头像，不再把已显示图片换回待下载 URL。通用素材替换同时取消 100ms 透明度过渡，刷新时默认图标、定制图标和头像均连续可见。",
    kind: "fix"
  }, {
    title: "素材库行情后台更新且排序列稳定",
    desc: "素材库股票页的默认首屏直接从服务端数据库输出上次持久化结果，不再先显示整页骨架；浏览器同时保存最后一次成功的筛选页面，刷新先恢复原表格再静默校验。任一证券缺少行情时只给热缓存极短的合并窗口，冷缓存继续在后台更新；翻页、筛选和排序期间保留现有表格。所有排序标题永久预留箭头宽度，点击市值等列时后续标题与市场列保持原位。",
    kind: "fix"
  }, {
    title: "素材库 URL 状态首帧水合一致",
    desc: "修复带 sort、market、page、q 或卡片视图参数刷新素材库时，服务端默认状态与客户端直接读取 URL 后的状态不同而触发左下角 Next.js Issue。分类、市场、排序、页码、搜索和卡片布局现在先以统一状态完成水合，再在首次绘制前恢复 URL 参数，保留持久化体验且不再产生 hydration mismatch。",
    kind: "fix"
  }, {
    title: "手机端全局壳层与高频操作区适配",
    desc: "系统收紧手机端顶栏、页面上下留白和页脚，窄屏隐藏占位较大的指数条；十多个主入口改为单行图标文字胶囊并支持横滑、自动居中与吸顶。素材库分类和市场筛选支持稳定横滑，搜索框独占一行避免挤压。自选分组管理在手机上改为分组栏与成员区两段可滚动布局，操作按钮可横滑，避免固定桌面高度导致内容或底部动作被裁掉。",
    kind: "fix"
  }, {
    title: "修复手机收益日历金额溢出",
    desc: "资产分析与资产盈亏分析共用的收益日历新增手机专用窄金额格式：千位起使用千、万、亿单位，小额去除无意义小数；375px 月视图采用更紧凑的字号和字距，日期格限制自身溢出。收益率根据数值长度自适应小数位，桌面仍保留原有精度和显示方式。",
    kind: "fix"
  }, {
    title: "调整手机收益日历比例",
    desc: "手机月历不再沿用桌面端 80px 日期格高度：窄屏日期格收紧为 52px，格间距与金额上下间距同步缩小，圆角按尺寸调整；整月高度明显降低，单格比例更接近移动端日历卡片，桌面布局保持不变。",
    kind: "fix"
  }, {
    title: "排查并修复手机端七列日历比例",
    desc: "系统检查收益日历、财报日历、日期区间选择器、订单日期筛选、资金日期选择器及交易有效期日历。收益日历加载骨架与年视图改为匹配手机实际高度，避免加载前后跳动；财报日历手机日期格从 88px 收紧为 52px，仅显示日期与财报数量，完整公司信息使用页面已有的下方日期详情区，避免在窄格中重复堆叠。桌面端继续显示公司摘要与悬停详情，其余日期选择器本身已采用方形日期格，无需改动。",
    kind: "fix"
  }, {
    title: "资产总盈亏二级页即时打开",
    desc: "修复资产分析点击持仓总盈亏后长时间显示骨架的问题。主页面与二级页的组合历史及基准 K 线窗口统一为 330 天，使主页面已经完成或正在进行的请求可直接命中同一份内存缓存；二级页首帧直接复用父页面现成的持仓、行情、组合收盘序列与订单，不再等待自身的持仓、行情和历史组合请求，趋势图与收益日历也可直接出现。客户端缓存有效期按实际接收时间计算，避免服务端数据时间较早导致刚请求完就被误判过期。独立地址进入时仍保留完整取数流程。浏览器实测点击约 260ms 即显示总盈亏、排行和明细，无骨架阻塞。",
    kind: "fix"
  }, {
    title: "新增全站上下文账户助手",
    desc: "全站右下角新增账户助手：桌面使用不遮挡主内容的右侧浮动面板，手机使用底部面板；自动识别账户资产、资产分析、资产总盈亏、自选股、FIRE、财报日历、卡面库和素材库等当前页面，并提供对应快捷问题。服务端复用设置中的大模型提供商、API 地址、模型与密钥，只向模型提供当前用户的受控持仓摘要，密钥不会下发浏览器，模型不能直接读写数据库；未配置模型时仍可在本地完成持仓概览和基础数据诊断。回答明确区分事实、限制与下一步；页面操作、分组整理与交易确认能力已在本版后续条目补齐。",
    kind: "feature"
  }, {
    title: "账户助手刷新后恢复上次对话",
    desc: "账户助手按用户在服务端保存最近 30 条消息，刷新页面、切换功能或换设备后重新打开仍可继续上次对话；旧版浏览器历史会自动迁移并清除，不同用户的历史互相隔离。标题栏新增“开始新对话”按钮，可随时清空历史重新提问；异常历史自动降级，不影响当前会话。",
    kind: "fix"
  }, {
    title: "账户助手支持页面操作、分组整理与交易预览",
    desc: "完成账户助手后续能力：可用自然语言打开功能页、筛选美股/港股/A 股自选，创建自选分组并将多个代码批量移入指定分组；买入或卖出指令先生成包含股票、数量、价格与预计金额的结构化预览，用户再次确认才会写入订单。分组移动和交易入账完成后均可在原对话中撤销，底层继续复用现有归属校验、持仓重算和订单账本。账户分析补充按市场集中度、重复代码、资料缺项及超过 7 天未更新记录的诊断；分组操作后会即时刷新行情板。修复助手跳转到市场筛选后刷新出现首帧跳动和 hydration 警告。",
    kind: "feature"
  }, {
    title: "账户助手真实场景稳定性加固",
    desc: "按刷新、浏览器前进后退、同页切换市场、快速连点和请求中重开会话等真实路径回归：助手从美股直接切港股时以新 URL 为准，不再被旧筛选状态改回；交易与分组确认预览增加 15 分钟有效期，旧版无时间戳预览自动失效；执行中刷新后的交易标记为待核对，避免网络结果不明时重复入账。确认与撤销统一串行并拦截双击；新对话会取消未完成回答并隔离旧响应；前端与服务端都只保留最近 30 条。服务端严格清洗持久化动作与撤销结构，旧浏览器历史只迁移文本。跨市场同代码不再默认取第一只，要求用 US: / HK: / CN: 明确市场，交易预览同步显示市场；批量分组遇到未识别代码时整体停止，不再静默只移动其中一部分。浏览器实测新交易预览刷新后仍可确认，旧预览不可点击，同页市场切换与历史返回正确。",
    kind: "fix"
  }, {
    title: "账户助手操作支持安全重试",
    desc: "交易、建组和移动分组统一改由服务端幂等操作入口执行，每个预览使用固定操作标识并在 SQLite 事务内完成业务写入与结果登记；网络中断或刷新后用同一标识重试只返回首次结果，不会重复下单、建组或移动，标识被篡改后拒绝执行，业务失败会整体回滚。撤销也使用独立固定标识并支持安全重试，超过 15 分钟后明确禁用。对话保存失败自动重试三次并显示可手动重试状态；手机底部面板补齐圆角、点击遮罩关闭，桌面自动聚焦输入框，支持 Escape 关闭和中文输入法回车保护。浏览器以 390px 视口实测刷新、历史恢复、面板滚动与关闭，未执行真实交易或分组变更。",
    kind: "fix"
  }, {
    title: "账户助手入口采用长桥配色",
    desc: "参考长桥 Olo 青的使用层级重做右下角账户助手入口：按钮使用沉稳石墨黑作为主体，白色星芒负责识别，仅在角落保留一颗带柔光的 Olo 青亮点；悬停只轻微提亮深色底和阴影，不再铺满高饱和青绿色。",
    kind: "fix"
  }, {
    title: "账户助手面板统一青绿色视觉",
    desc: "打开后的工作面板固定使用清爽白底，在深色站点中也保持一致；长桥 Olo 青缩减为点睛色：用户消息接近白色，只保留极浅青边，文字恢复中性深灰；发送按钮改为浅薄荷底与青色箭头，去掉大面积实心青色。标题图标保持与入口完全一致的石墨黑圆底、白色主星和青色小星，只按标题栏缩小尺寸。",
    kind: "fix"
  }, {
    title: "账户助手换用长桥风格星芒图标",
    desc: "移除线条偏碎的通用 Sparkles 图标，改为专门绘制的饱满四角白色主星与右上 Olo 青小星组合；入口与面板标题共用同一个图标组件和同一套石墨黑圆底配色，只通过尺寸适配不同场景，确保两处长期一致。",
    kind: "fix"
  }, {
    title: "账户助手隐藏页面识别状态",
    desc: "标题栏移除“正在查看：财报日历”等当前页面状态，只保留账户助手名称；输入框下方同时移除示例命令与数据用途两行常驻说明，保存正常时保持留白，仅在失败时显示可点击的重试提示。页面上下文仍仅在发起提问时作为受控信息用于生成相关回答。",
    kind: "fix"
  }, {
    title: "账户助手支持多会话历史",
    desc: "每次新对话生成独立会话 ID，服务端按用户最多保存 20 个会话并自动迁移旧版单会话历史。标题栏新增历史图标，抽屉中展示标题、短 ID 与更新时间，可切换或删除单个会话；新建对话不再清空旧记录，刷新后恢复最近使用的会话。发送按钮参考素材库市场徽标改为浅色圆盘与青色核心点缀，可发送时显示低频扩散光环，悬停时箭头轻微上移，并尊重系统减少动态效果设置。",
    kind: "feature"
  }, {
    title: "市场与国家图标移除 Emoji 回退",
    desc: "修复素材库市场图标在自定义素材失效或默认文件缺失时退回 Emoji 的问题。全站 MarketIcon、素材库国家/地区旗帜和全球经济地图统一改用本地素材优先、内置矢量地球兜底；持仓市场编辑器移除 Emoji 国旗输入与默认值，新市场图标只从素材库解析。新增回归约束，防止界面再次引入 Emoji 图标回退。",
    kind: "fix"
  }, {
    title: "账户助手补齐真实页面上下文与失败恢复",
    desc: "账户助手不再在卡面库和 FIRE 页面只拿股票数据作答：卡面库会读取当前用户持卡数与按原币汇总的录入金额，并明确金额可能含信用卡额度；FIRE 只读取允许的规划参数；素材库会检查分类数量与本地文件缺失项，敏感卡号、安全码等信息不会进入上下文。回答支持标题、列表、粗体和行内代码排版；输入框随内容增高；用户上滑阅读时新回复不再强制拉到底。请求失败显示可持久化的重新回答按钮；删除会话会等待保存队列并阻止旧请求把已删除会话重新写回，Escape 优先关闭历史抽屉。",
    kind: "fix"
  }, {
    title: "账户助手安全边界加固",
    desc: "账户助手问答、会话保存和确认操作改为流式限长读取请求体，阻断超大 JSON 在解析前占满内存；页面名称改由服务端白名单确定，股票和筛选字段严格限长限字符，账户数据以不可信 JSON 区块交给模型并明确忽略其中的提示注入。模型地址只允许无凭据的 HTTP/HTTPS，拦截云元数据与链路本地地址；供应商错误不再透传，回答统一限制 4000 字。交易、分组和撤销补齐标识长度及数量、价格、费用、名义金额上限，未知内部错误统一收敛。Next.js 与 Sharp 升级至安全版本；移除存在原型污染和 ReDoS 且无修复版本的 SheetJS，账本 Excel 导入导出迁移到 ExcelJS，并在解压前限制压缩包条目、膨胀率和解压总量。既有同源校验、用户隔离、确认执行、15 分钟有效期与 SQLite 幂等事务继续生效。",
    kind: "security"
  }, {
    title: "智能助手新增 Harness 风格模型轨迹",
    desc: "直接参照 DeepSeek Harness 开源源码移植完整智能助手界面：独立工作台采用固定会话侧栏、单一会话画布与底部输入坞；输入框复刻 22px 圆角、浮层模型/数据菜单、工具行、运行统计、状态按钮及弹出动画。浮动入口恢复最初的轻量面板，底部使用横向铺满的单行输入胶囊，仅保留输入和发送按钮；发送按钮始终使用 ChatGPT 风格实心绿色，回答中显示纯白停止方块，继续输入时恢复白色箭头。等待模型首段输出时以低对比度文字流光呈现思考状态，正文开始输出后自动消失，并适配深浅色与减少动态效果。用户消息默认隐藏操作图标，双击后直接在原气泡内修改并重新回答，编辑态仅显示取消与保存。气泡使用更轻的 0.5px 描边和紧凑内边距。工作台侧栏恢复 Harness 的工作区搜索行与收窄按钮，右上角三点菜单集中管理对话记录和归档；主输入区移除重复的调用、Token、服务状态、数据范围、临时对话、发送内容与导出控件，仅保留附件、模型和发送，浮层点击外部自动关闭。侧栏同时复用资产分析的蓝色可拖动分隔条，可在 220–420px 间调宽、双击恢复默认并按账户保存。宽度持久化在浏览器存储满额或不可用时安全降级为 Cookie，避免界面崩溃；拖动期间关闭列宽过渡，消除 280ms 追赶延迟。工作台或助手设置中当前选择的模型写入账户偏好，并同步用于各页面浮动入口。会话右侧增加轮次短线轨道。新增 800×800 设置中心，包含通用设置、对话与记忆、模型、插件和助手预设；空间、记忆和模型调用统计集中进入设置。空间选择采用自定义浮层。会话列表支持重命名、归档、恢复和删除，手动标题不会被后续保存覆盖。工作台、浮动面板、设置中心及其浮层统一服从助手自身主题。模型服务可在当前弹窗内选择和编辑，全程不跳转。轨迹页保留 Input / Model / Tools 三泳道、高密度调用列表及右侧详情。记录按用户和对话隔离，删除对话同步清理，且不保存 API Key、请求头、账户摘要、问题正文、回答正文或图片内容。",
    kind: "feature"
  }, {
    title: "新增智能助手独立工作台",
...
export const V0_1_31_ENTRY: VersionEntry = {
  ...
  changes: [{ title: "公开仓库安全加固与素材分离", desc: "SVG 使用 XML 解析白名单，上传路径与真实图片格式校验；请求体流式限额，助手附件限额，财报权限保护，首次安装令牌及非 root 容器运行，保留图片优化缓存写入权限并兼容历史清理后的提交检查。公开代码采用 MIT，部署素材由部署者自行提供。", kind: "security" }, {
    title: "四色门工作区入口迁入主站",
    ...
    title: "业务页面恢复原有内容结构",
    desc: "移除业务内容上方重复的页面名称、分隔线与四色门状态顶栏，各页面直接从自身内容开始，保持持仓宽表、资产分析分栏与管理页面原有空间；矮窗口补足手势与侧栏分隔线的间距。智能助手按顶部栏以下的真实视口高度自适应，助手页不再渲染站点页脚，桌面端与移动端均完整保留对话和输入区域。",
    kind: "feature"
  }, {
    title: "四色门音效按钮去掉绿色状态标识",
    desc: "四色门音效开关右下角的绿色状态圆点（含静音时的灰色圆点）一并移除；同时取消开启态的绿色基调，深色模式下按钮不再有绿色背景块与绿色描边，浅色模式也不再染成墨绿色图标，按钮回到统一的中性配色。开启与关闭仍通过喇叭图标本身区分（关闭为带叉喇叭），深浅色表现一致。",
    kind: "fix"
  }, {
    title: "智能助手工作区与左侧导航等高",
    desc: "桌面端智能助手页面此前把工作区高度锁在 680px，窗口较高时右侧面板下方会空出一大块，与左侧主导航的底部对不齐。现在 1024px 以上改用与左侧导航完全相同的视口高度（100vh - 112px，同一顶部起点 88px），两侧面板上下边缘严格对齐，对话区与输入框随窗口高度自适应；对话过长时仍在面板内部滚动，不产生整页滚动。手机与平板宽度保持原有紧凑高度。",
    kind: "fix"
  }, {
    title: "主导航分组名改为「资产」",
    desc: "左侧主导航第一个分组标题由「工作区」改为「资产」，与分组内的持仓、资产分析、自选股、FIRE、全球经济保持一致；下方「管理」分组不变，智能助手侧栏的对话分组标题也不受影响。",
    kind: "fix"
  }, {
    title: "深色模式智能助手输入框配色修正",
    desc: "深色模式下全局表单聚焦填充（#151a26 蓝黑）会盖到智能助手的输入框上，工作台底部的输入框因此变成一块与卡片底色（#2b2b2b）不协调的深蓝色块。现在助手内的输入控件聚焦时保留自身底色：底部输入框与卡片一致，带独立底色的会话改名 / 空间名输入框和模型编辑器字段维持各自配色，悬浮面板与深色助手主题一并生效，浅色模式不受影响。",
    kind: "fix"
  }, {
...
export const V0_1_32_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "智能助手对话列收窄居中",
    desc: "宽屏下对话正文此前横向拉满整个工作区，单行过长、读起来累。现在消息列收窄到 768px 并水平居中，底部输入卡片与用量统计行同步收窄到同一宽度，三者左右对齐；轨迹视图仍按原设计通栏铺满，手机与窄屏不受影响。",
    kind: "fix"
  }, {
    title: "轮次短线对齐主流大模型的对话轨道",
    desc: "对话右侧的轮次短线按「从上到下由旧到新」，但打开对话时高亮却固定停在第一轮（最上面），看起来像最新一轮在上面。现在打开对话或新增消息后高亮自动落到最新一轮（最下面那条）。样式与动效参考主流大模型软件的对话轨道：右对齐短横线、每根长度按 8 根一组的平缓旋律不同（12–20px）、当前轮更亮且只比邻条略长 4px（悬停多 8px，不再拉成 32px 的长条），静止时完全不动，只在悬停 / 切换时走 150ms 过渡。悬停预览也按主流做法收窄：卡片由 360px 单行改为 260px、正文两行截断，字号与内边距同步收紧；预览从按钮左侧展开、轨道不做裁剪，不会被切掉。",
    kind: "fix"
  }, {
    title: "交易广场刷新先出内容、头像不再丢",
    desc: "交易广场此前完全依赖浏览器本地缓存 + 接口返回：缓存不可用（无痕、配额满）或接口慢时，刷新会先渲染一整屏骨架，两位作者的帖子头像也要等接口才出现，看起来像头像丢了。现在把服务端已有的帖子快照（每位作者最近 40 条，含中文译文与本地化图片）随首屏下发，刷新时列表与头像立刻在位上；名人头像同时在 HTML 阶段并行预加载，不再先闪首字母。接口返回后照旧合并最新内容并写回本地缓存，筛选、翻页、未读角标逻辑不变。",
    kind: "fix"
  }, {
    title: "上传素材改为先缓存、再静默更新",
    desc: "名人头像、股票图标等上传素材此前带 Cache-Control: max-age=0，每次刷新都要重新校验/重传（名人头像单张接近 1MB），页面上会先显示首字母占位，看起来像头像丢了。现在 /uploads/* 统一返回 public, max-age=3600, stale-while-revalidate=86400：一小时内直接复用浏览器里上次成功加载的副本，刷新瞬间出图；过期后先显示旧图、后台静默校验，有更新再自动换新（上传文件名带时间戳，换头像本身也会换 URL）。静态命中与运行期上传两条服务路径都已覆盖。",
    kind: "fix"
  }, {
    title: "图片组件不再先闪一次占位",
    desc: "SafeAssetImage（导航、市场、货币、股票图标与名人头像统一走的图片组件）此前把 <img> 初始设为 opacity-0、等 onLoad 后才切到可见，于是即使图片已经在浏览器缓存里也要多等一帧，刷新时仍会先闪一下首字母或默认图标。现在图片始终参与绘制、占位垫底：命中缓存时首帧就是真图，尚未加载完成时图片透明、占位自然透出；加载失败依旧回退占位，且 alt 为空不会出现破图图标。交易广场与名人持仓的名人头像预加载同时加上 fetchPriority=high，尽早开始取图。",
    kind: "fix"
  }, {
    title: "交易广场股票提及改为雪球样式",
    desc: "正文里的股票提及现在按雪球习惯渲染为蓝色链接 $名称(代码)$：原文里的「西方石油」、单独的「OXY」，以及雪球原文自带的 $西方石油(OXY)$，统一显示成 $西方石油(OXY)$，颜色取雪球蓝 #3774e0（深色模式 #5b91f0）。原有的虚实线下划线设定保留：线下划线的颜色仍按实时涨跌着绿 / 红，未取到行情时用品牌灰；悬停只提亮文字、不改变虚线下划线，点击仍打开行情卡片与个股详情。",
    kind: "fix"
  }, {
    title: "交易广场刷新不再闪掉筛选行",
    desc: "交易广场的筛选状态（看谁 / 分类 / 页码 / 个股）此前只写在 URL 里，刷新时服务端首帧只能画出「全部动态」，水合后才补上「段永平 + 全部 / 热门 / 原发 / 长文」那排标签 —— 于是这排标签每次刷新都先消失再出现。现在把同一份状态镜像到 cookie（fire_trading_square_filter），服务端首帧就用它渲染，标题、条数、标签行、页码与个股详情刷新后原位不变；URL 仍是最终依据，cookie 只作首屏提示，清除 cookie 即回到默认。",
    kind: "fix"
  }, {
    title: "修复股票提及误匹配与重复链接",
    desc: "特朗普那条帖子的中文译文里「纽约州总检察长和曼哈顿地区检察官」被识别成港股「长和(00001)」，正文凭空多出一个股票链接 —— 根源是两字中文名匹配时没有边界判断。现在两字中文名要求左右至少一侧不是汉字才算提及：「我买了长和」「长和(00001)」照旧命中，「检察长和曼哈顿」不再命中；雪球原文的 $名称(代码)$ 写法不受影响，仍可出现在句子中间，四字及以上中文名保持原行为。同时修掉裸写「名称(代码)」会连出两个重复链接的问题：相邻且同标的的名称与代码命中会合并成一个提及，并吞掉多余的右括号，输出恒为 $长和(00001)$。回归用例已补进 tests/site-review.cjs。",
    kind: "fix"
  }, {
    title: "补齐公开历史里的个人标识清理",
    desc: "上一轮公开仓库清理只覆盖了工作区与部分规则，本轮把「可达历史」补齐：版本记录、部署文档、构建说明与用户表迁移脚本里残留的个人标识（GitHub 账号 / 镜像路径、登录名、本机代理端口与本地代理软件名、NAS 硬件型号）统一替换为中性占位，本地与远端 main 的提交历史一并重写，旧提交不再被任何分支或标签引用。仓库对象库从 1.3GB 降到 16MB，本地 2257 个可回收对象与 13 个游离对象全部清除；工作区文件、提交信息与全历史 blob 扫描均无命中。公开仓库上清理前上传的 44 个构建记录（Actions artifact）已删除，容器镜像包仍不可匿名拉取，工作流日志不含个人标识。遗留说明：GitHub 对不再被引用的旧提交可能短期保留按 SHA 的缓存视图，需要时可向 GitHub Support 申请清除。",
    kind: "security"
  }, {
    title: "启动脚本自检挂载目录权限，种子失败不再阻塞启动",
    desc: "线上（群晖）升级到非 root 镜像后出现过容器无限重启：DATA_DIR / UPLOADS_DIR 是宿主机 bind mount，属主不是容器内的 uid 1000（node 用户），启动脚本往数据卷写公开缓存种子时报 EACCES，而 entrypoint 带 set -e 直接退出，被 restart: unless-stopped 反复拉起（日志里只有一行英文报错，很难判断原因）。现在 entrypoint 启动前先自检两个挂载目录的写权限：数据目录不可写时打印明确的中文提示（含 chown -R 1000:1000 data uploads 这行命令）后再退出；上传目录不可写只告警、服务照常启动；公开缓存种子合并失败一律只告警，不再阻塞启动。部署文档补上「挂载目录属主必须是 1000:1000」的前置步骤，以及「升级后容器无限重启、日志只有 EACCES」的完整排查章节。部署一致性审计改为校验「镜像内路径必须被启动脚本引用」这一不变式（配合路径变量化），并补了一条回归用例同时覆盖「数据目录不可写必须报错退出」与「种子失败不阻塞启动」。",
    kind: "fix"
  }, {
    title: "四色门侧栏素材随仓库与镜像提供",
    desc: "四色门侧栏依赖的 4 个素材（窗户 / 转盘 / 指针 / 手型光标）此前被排除在仓库和镜像之外，只在开发机本地存在；线上或新克隆环境里 window.png 404 会命中组件的 onError 回退，侧栏入口先闪一下再整块消失。现在把这 4 个文件纳入版本管理（.gitignore 精确放行，其余 uploads 与二进制素材仍然禁止入库），构建上下文与镜像一并携带（.dockerignore 同步放行，进入 /app/resource-default 后由 entrypoint 播种到 uploads 卷，老部署重建容器即可补齐），公开仓库审计对这份白名单放行、其他规则不变，ASSETS.md 与群晖部署文档同步说明。未使用的 room.png 仍留在本地不入库。",
    kind: "fix"
  }, {
    title: "弹窗底部按钮统一为胶囊样式",
    desc: "弹窗底部按钮此前只挂了 .btn（纯布局）和一个并不存在的 btn-secondary 类：取消按钮因此没有任何底色与文字色，深色模式下几乎看不见；删除类确认按钮也缺内边距，看起来像一行红字而不是按钮。现在在 globals.css 制定一套共享的弹窗胶囊配方 —— .dialog-actions（右对齐 + 10px 间距）+ .dialog-btn 三档语义：ghost（取消 / 次要，透明底、主题自适应文字色、悬停浅灰）、neutral（确认 / 保存，白底浅灰边框，深色模式深灰底白字）、danger（删除，实心红底白字），统一 38px 高、全圆角。AppDialogHost、导入截图弹窗、名人管理弹窗、卡包 / 卡面弹窗以及助手「重命名 / 新建空间」弹窗（原本各自手写 rounded-xl / btn-line / 自定义 CSS）全部改用这套配方，深浅色对比度一致；顺带比对源码用到的 btn-* 类与 globals.css 定义，确认不再有「用了但没定义」的按钮类。",
    kind: "fix"
  }, {
    ...
    desc: "以前助手只会收到系统提示和对话本身：服务端算出的页面上下文与页面数据快照只被本地兜底回答使用，模型那边一条都没拿到，所以问「最近有什么大新闻」只能回「我没有联网、没有你的页面数据」。现在新增 lib/assistantLiveData.ts，把站内本来就实时抓取的数据整理成快照随提问一起下发：交易广场最近动态（特朗普 Truth Social / 段永平雪球，含中文译文摘要、作者与时间，可当站内新闻源引用）以及我的持仓 / 自选行情（原币现价与涨跌幅，仅在助手「账户数据」范围内、且提问与行情相关时才去抓，2.5 秒硬超时、失败即跳过，不阻塞对话）；同时把页面、个股、筛选、页面数据快照、账户记录数组成「站点上下文」块，作为独立 system 消息真正发给模型，并明确引用口径：可据此回答并注明抓取时间与来源，块外仍不得编造，也不得声称联网搜索过；问不到时直接说明「站内暂无可用的××数据」并指路到对应页面。没有配置大模型时的本地兜底回答同样会把交易广场动态摊开回复，不再只回一句「大模型尚未配置」。",
    kind: "feature"
  }, {
    title: "浮动入口补齐管理类页面上下文",
    desc: "用户管理 / 附件管理 / 日志此前不在助手页面白名单里（客户端 PAGE_COPY 与服务端 normalizeAssistantContext 都缺），于是在这些页面打开浮动助手，它只会说「当前页面上下文为空」。现在两处白名单都补上这三个页面，并各自给出页面数据快照：附件管理（根目录文件 / 目录数、最近 5 个文件）、用户管理（用户总数与角色分布，不含任何凭据）、日志（最近 5 条安全事件的时间与类型）；同时补上交易广场 / 全球经济 / 设置的快捷提问，让浮动入口在这些页面也能直接问出有用的东西。",
    kind: "fix"
  }, {
    title: "助手回答支持表格与可点链接",
    desc: "回答渲染此前只认标题、列表、代码和粗体：模型最常用来报数字的 Markdown 表格会整段以裸竖线显示，正文里的链接也只能当纯文本看。现在把 Markdown 解析抽到 lib/assistantMarkdown.ts（纯函数，便于回归测试），补齐表格、引用块、分隔线三类块与链接、行内代码、粗体三类行内样式：表格以带表头、隔行底纹的样式渲染，外层可横向滚动，手机上也不会把对话撑宽；链接只在 http/https 协议下渲染成可点链接（javascript:、data: 一律按纯文本显示，避免点出脚本），沿用交易广场的雪球蓝并带下划线，新标签页打开。回归用例覆盖表格解析结果、非法协议被拒绝，以及「渲染层确实接上了这两类块」。",
    kind: "feature"
  }, {
    title: "长对话补上「回到最新」并收紧回答长度",
    desc: "对话变长后向上翻看时，此前没有任何回到末尾的入口，只能拖滚动条；同时助手的回答容易写成大段套话（开场先声明「我这边没有…」、再罗列一串「我可以帮你做什么」）。现在消息列表在离开底部时浮出一颗「回到最新」胶囊（贴底状态由滚动、发送、切换对话统一走同一个入口维护，只在状态真的翻转时才触发渲染），点击平滑回到底部并自动隐藏；系统提示同步补上回答风格约束：默认短、先给结论、整套控制在 8 行以内、不要复述问题、不要用免责声明开场、不要罗列能力清单、要比较多组数字时用短表格或一行一项的短列表。",
    kind: "feature"
  }, {
    title: "修复线上 FIRE 页面「小丑鱼不见了」",
    desc: "线上 FIRE 页面那对游动的小丑鱼（父子鱼）与礁石贴图整体消失，页面本身不报错。根因是插图 / 字体 / 图标 / 分享图按素材授权约定既不进 Git 仓库、也不进镜像（.gitignore / .dockerignore 排除），而 Compose 只挂了 data 与 uploads —— 部署素材没有任何进入运行时的通道，线上 /images/fire/clownfish-family.png 必然是 404；本地 dev 直接读源码目录下的 public/，所以本地一切正常，容易误判成代码 bug。现在 Compose 补上四个只读挂载（./images、./fonts、./icons、./share → /app/public/*，可用 IMAGES_DIR / FONTS_DIR / ICONS_DIR / SHARE_DIR 覆盖），启动脚本在素材缺失时打印一行中文提示（服务照常启动），部署文档新增「1d. 挂载插图 / 字体 / 图标素材」与「FIRE 页面的小丑鱼不见了 / 图片、字体、图标 404」排查章节，部署一致性审计把四个挂载与启动提示列为必需项，回归用例覆盖「缺失时有提示、补齐后安静」。",
    kind: "fix"
  }, {
    title: "回退持仓盈亏图的排版与配色改动",
    desc: "按要求把「我的持仓 → 持仓盈亏图」这一轮的改动整体回退到调整前的状态：删掉按宽度自适应的排版模块（lib/sankeyLayout.ts）与图表配置模块（lib/sankeyModel.ts），components/HoldingsPnlSankey.tsx 与 app/globals.css 恢复为原版本 —— 手机端回到「左右各留 82px、柱宽 10px、两行 9px 标签、画布高 392px」的原始排版，配色仍是纯色盈利红 #e5484d / 亏损绿 #0aa77d / 中间汇总紫 #7567b9 与单色半透明连线（曲率 .5、透明度 .55）。上一轮为此新增的 4 条版本记录（宽度自适应、渐变观感重做、去压扁、柱体加宽）与 2 个回归用例一并撤掉，回归套件回到 27 条；工作区与调整前的提交逐文件比对已无差异。",
    kind: "fix"
  }, {
    title: "恢复相关 ETF 角标为改动前的样式",
    desc: "按要求撤回这一轮对角标的全部改动，恢复到用户提出「角标太压抑」之前的状态：components/EtfDoubleBadge.tsx 还原为原来的 19px 深灰实心圆角标（#4b5563 底、8px 加粗白字、1.5px 表面色描边、轻投影，定位在图标右下 -bottom-2 -right-0.5，2x 保留小写 x），8 个页面共 10 处调用点也回到「标记挂在图标容器内」的写法；期间尝试过的浅色小胶囊、以及「名称下方代码行内联标签」两版一并撤销，回归用例里针对内联标签的那条断言同步删除（套件回到 27 条）。工作区与提出该需求前的提交逐文件比对已无差异（只剩这条版本说明）。",
    kind: "fix"
  }, {
...
export const V0_1_33_ENTRY: VersionEntry = {
  ...
  changes: [{
    ...
    title: "四色门转动更顺，落位音改为叮",
    desc: "转盘旋转从每帧重绘的 drop-shadow 改为圆形 box-shadow，避免滤镜跟着转导致卡顿；短转 820ms、随机长转 1760ms，缓动加长滑行。落位去掉低频咚，改成 C7 高亮叮（三角波 + 泛音），棘轮声仍跟转角走。",
    kind: "fix"
  }, {
    title: "珐琅转盘红区箭头改回旧图样式",
    desc: "样式 2 红色扇区的箭头改为与样式 1 插画转盘相同的左指弯箭头：右端圆头、弧线呈笑嘴、左端实心三角箭头，贴在红区中下部。",
    kind: "fix"
  }, {
    title: "四色门一次转到落点，不再拧回起点",
    desc: "随机转此前多转几圈后落点与真正打开的工作区可能不一致，转完又被 CSS 把 -810° 收成 -90°，看起来像倒转回起点。现在随机转两整圈后直接停在目标色块，转完无动画把角度收进一圈；阴影改在不转的槽上，挂钩不再跟着抖，棘轮声按缓动曲线推算、不再每帧读样式。",
    kind: "fix"
  }, {
    title: "四色门音效收软：落位叮更悠、转动不再刺耳",
    desc: "落位叮去掉 C7 三角波和 4kHz 泛音，改成 G5 正弦钟（约 0.6 秒衰减、20ms 起音）。棘轮声从 1.2–1.6kHz 降到约 540–620Hz，转得越快越轻越低，并加 28ms 最小间隔与 2.4kHz 低通，避免一帧叠多记造成尖啸。",
    kind: "fix"
  }, {
    title: "四色门音效改成转前 / 转动 / 落定三段风铃",
    desc: "对照霍尔移动城堡四色盘「开始转、转的过程、停住」的听感重做合成（不取样原片）：起手是上行的 G-B-D 碎晶，转动中按五声音阶轮换风铃并重叠余响，停住再落一段 E-B-G-D 下行收束，约 1 秒余响。全是正弦、柔起音，避免再出现干脆的电子哔。",
    kind: "fix"
  }, {
    title: "四色门音效改成纸牌拨动感",
    desc: "对照原片转盘听感后，风铃合成仍差一截——转动更像一叠卡片被飞快拨过，而不是钟。现改为噪声带通的纸牌拨动：动手两下 fwip，转起来按 15° 一记沙沙（越快越密越薄），停住是最后一记卡片贴合加很轻的木质卡住。不取样原片。",
    kind: "fix"
  }, {
    title: "四色门音效按原片频谱改成宽带卡片拨动",
    desc: "对原片转盘片段做了频谱：转动是约 10–12Hz 的竖直宽带脉冲（能量从几百赫到 8kHz），不是音高钟声。合成改为短噪声闪击（中频 2.2kHz + 塑料感 5.2kHz），起手相隔 80ms 两记，转动最小间隔 65ms 以免连发成拉链，落定只加一记更实的拨动、去掉所有正弦叮。不把原片音频写入仓库。",
    kind: "fix"
  }, {
    title: "四色门音效回到风铃碎晶，对齐花园那场转盘",
    desc: "宽带卡片版听感比之前的风铃差。对着花园那场「See that new color on the dial」原片从 02 秒抽了音轨：转前是 4.1–4.4kHz 一串碎晶（约 40ms 一记），转动中是 2.4–3.8kHz 更轻的重叠风铃，落定是 4.2→3.5→4.7kHz 下行收束。合成改回正弦风铃三段（基音+微失谐+低八度身），不再用噪声拨片；不把原片音频写入仓库。",
    kind: "fix"
  }, {
    title: "四色门转动中保留之前的五声风铃滑动",
    desc: "转前碎晶和落定下行仍按花园那场对齐；转动过程改回此前的 G-B-D-E-G 五声音阶风铃（带轻八度），随转速变密变轻，不再用偏高的碎晶叠音。",
    kind: "fix"
  }, {
    title: "四色门音效回到线上棘轮再往下落定收软",
    desc: "风铃 / 纸牌几轮都偏了，音效先回到线上版那套三角波棘轮（约 690→410Hz、高通 360Hz、45ms）。转动仍按转角发同一套滑动，转前补线上那记 0.1 秒延迟的第一下。落定保留线上 G6/G7/D8 钟和 4.2kHz 轻咔，去掉最刺的 6kHz。样式 1/2、挂钩和落点不变。",
    kind: "fix"
  }, {
    title: "四色门音效改成珐琅八音盒",
...
export const CURRENT_VERSION_ENTRY: VersionEntry = {
  ...
  changes: [{
    title: "登录二次验证（TOTP）",
    desc: "设置 → 二次验证可绑定验证器。扫描本地生成的二维码（密钥不经过第三方图片接口），输入 6 位数字后开启，并一次性给出 8 条备用码。开启后，网页与 iOS 登录在密码正确时不会立刻发会话：先返回 5 分钟有效的 ticket，再提交验证码或备用码才登录。同一验证码不能重放；管理员重置密码会同时关闭该用户的二次验证，用户管理页也可单独关闭。",
    kind: "security"
  }, {
    title: "二次验证改成独立设置项",
    desc: "二次验证从设置 → 个人信息里拆出来，账号分组下单独一项，地址为 ?sub=totp。个人信息仍可在改密时填写验证码。",
    kind: "fix"
  }, {
    title: "二次验证安全加固",
    desc: "备用码加长到 64 位熵并用数据密钥 HMAC 存储；TOTP 密钥落盘 AES-GCM 加密。登录 ticket / 绑定过程改存 SQLite，多进程不会丢，同一账号只保留最新一张 ticket，错 8 次作废。开启二次验证会踢掉其它会话；改密与管理员关闭二次验证都要再验一次。用户列表不再查出密钥字段。",
    kind: "security"
  }, {
    title: "四色门两套样式各用各的音效",
    ...
    title: "ECharts 地图与 K 线拆包，浮动助手空闲后再挂",
    desc: "核心图表包去掉 MapChart 和默认捆绑的 SVG 渲染器，全球经济热图单独走 lib/echarts-map.ts。持仓 / 自选打开时不再带上世界地图。浮动助手改为 next/dynamic，首屏空闲后再挂载，不挡持仓首屏。图表和助手外观不变。",
    kind: "fix"
  }, {
    title: "交易广场接入帖子评论（含评论者头像）",
    desc: "段永平把雪球设成「只有关注的人能评论」，所以他的帖子评论很少（实测最新 80 条里最多 4 条），但评论是正文之外唯一能看到别人观点的地方。现在新增 lib/tradingSquareComments.ts：用配置里的雪球 Cookie 调 statuses/comments.json 抓评论（每条帖子第一页、最多 20 条），按帖子 id 存进 data/duan-comments.json；刷新时「评论数没变 + 缓存不超过 30 分钟」就跳过，不额外打接口。界面上一级页面保持清爽：帖子底部只多一个「评论 N」图标按钮，点开才展开评论列表（头像 / 名称 / 时间 / 赞数 / 「回复 @某人」/ 正文），超过 3 条给「去雪球查看」入口。顺带修掉三处相关问题：1) 评论者头像取不到 —— 雪球的头像字段是「不带域名 + 逗号分隔多档尺寸」，现在取第一档并补上 xavatar.imedao.com，同时纳入图片白名单后下载本地化（实测 11/11 生效）；2) 引用的帖子没有作者头像 —— 引用对象新增 avatar 字段并本地化（17/17 生效），「有引用但缺头像」也纳入详情补抓；3) 正文里被清掉的「回复@某人:」现在以「回复 @某人 的动态」提示行保留在正文上方，既干净又不丢上下文。回归用例覆盖头像地址解析与白名单、评论字段映射（作者 / 时间 / 赞数 / 回复对象 / 正文）。",
    kind: "feature"
  }, {
    title: "交易广场卡片不再自己跑到中间",
    desc: "交易广场那张可拖动卡片是「居中 + 位移」实现的：面板由 mx-auto 居中、再用 translate 偏移，位置存在 localStorage。问题出在「把偏移夹回内容区」这段逻辑 —— 它会把夹过的值写回 localStorage：只要某个时刻内容区变窄（换显示器、窗口缩小、窄屏打开），偏移就会露到内容区外，夹紧后把「靠中间的值」永久写进存储，之后再打开就一直停在中间（用户看到的正是「过一段时间卡片跑到中间」）。现在把「用户位置」当作唯一真相：夹紧只作用于渲染、绝不回写 localStorage，只有拖动结束才写一次位置；同时补上布局变化监听（窗口 resize + 内容区 ResizeObserver），换显示器或侧栏收放后重算一次夹紧，窗口变宽后卡片会回到用户放的位置。拖动边界也改成用「居中基准矩形」计算（拿带位移的矩形反复叠加会吃掉位移）。回归用例覆盖「宽内容区放行 / 窄内容区只夹显示 / 变宽后回到原位置 / 内容区比面板还窄时贴左」，以及「夹紧不得持久化」这条不变式。",
    kind: "fix"
  }, {
    title: "交易广场正文清掉抓取带进来的页面文案",
    desc: "交易广场的帖子正文是连页面元素一起抓下来的，之前会把雪球页面上的链接文字当正文显示：1) 每条回复开头的「回复@某人:」（雪球把它渲染成正文前面的一条链接）—— 275 条段永平帖里有 171 条带这个前缀，而且紧接着就是「//@同一个人:」，同一个名字连着出现两次；2) 图片的链接文案「查看图片」（19 条；图片本身我们直接渲染，留着像是作者写了一句废话）；3) 外链的链接文案「网页链接」（4 条；真实 URL 抓不到，照抄出来是一句莫名其妙的话）。现在 lib/tradingSquareText.ts 新增 stripTradingSquareChrome()，并在 normalizeTradingText() 里统一调用 —— 抓取时与渲染时都会走到，所以已经躺在本地缓存里的老帖刷新后同样干净；正文中间的「//@某某:」转发链、「[已修改]」「[22:05]」这类属于原帖内容的标记一律保留。附带修掉一个连带问题：抓取脚本原本靠「正文以 回复@ 开头」判断这帖是不是回复、要不要去补抓被回复的引用，正文清洗后该判断会失效 —— 现在改为在清洗前记录 DuanPost.reply 标记（老缓存用 //@ 兜底）。助手的站内快照也改走同一套清洗，避免模型把这段页面文案当正文照抄。回归用例覆盖三类页面文案、正文中间回复链必须保留，以及「清洗前记录是否回复」这一不变式。",
    kind: "fix"
  }, {
    title: "评论展示改为主流社交网站的回复流",
    desc: "评论不再套一个圆角卡片框：改成主流社交网站（X / Reddit / YouTube）的回复流样式 —— 顶部一条细分隔线 + 「评论 N · 全部评论 ↗」，每条评论自成一行为「28px 头像（缺失时回退首字母）+ 名字（加粗）· 时间 + 正文 + 赞数」，楼中楼用「回复 @某人」标出；超过 3 条给「还有 N 条评论，去雪球查看 ↗」。一级页面依旧只显示「评论 N」图标按钮、点开才展开，保持列表清爽。",
    kind: "feature"
  }, {
    title: "评论按数量分流：十条以内内联，更多进二级评论页",
    desc: "评论展示按数量分流（参考主流社交网站的做法）：不超过 10 条时在帖子下面直接展开，每条是「头像 + 名字 · 时间 + 正文 + 赞」；超过 10 条时「评论 N」图标变成入口，点开进入二级评论页 —— 顶部是原帖（作者 / 时间 / 正文 / 配图 / 被回复对象）+ 返回按钮 + 「共 N 条 · 去雪球查看」，下面是完整评论列表。同时修掉评论头像与名字行没有对齐的问题（此前头像在整块里垂直居中，看着像错位，现在统一顶对齐）。数据侧把评论抓取从「只抓第一页」改成「评论超过 20 条的帖子最多抓 3 页（60 条）」，让二级页有足够内容可看；一级页面依旧只有「评论 N」图标，保持清爽。",
    kind: "feature"
  }, {
    title: "修复特朗普动态不再更新（归档站改版后解析静默失效）",
    desc: "交易广场里特朗普一侧停在 9/4 不动、段永平正常更新到 9/17。排查结论：抓取本身没问题（页面 200、内容都在），是归档站（trumpstruth.org，现在 301 到 www）改了标记，两处解析静默失效 —— 1) 日期改成包在 <time datetime=\"…\"> 里，旧正则要求日期文本紧跟 class=\"status-info__meta-item\">，取不到日期后该条被 date && (text || images) 过滤，整页解析成 0 条，刷新等于没刷新；2) 正文容器加了 data-post-preview 属性，旧正则要求 class=\"status__content\"> 严格匹配，于是新帖正文全空（老帖因为缓存里已有正文所以看不出来）。现在日期优先读 datetime 属性（保留旧写法与通用日期串兜底），正文改成按 div 深度配对提取（不要求属性完全一致，也不会被正文里的内层 </div> 截断）。顺带修掉一个连带故障：单页抓取超时（原 4 秒）会把整次刷新抛异常、一条都写不进去 —— 现在按页容错并放宽到 8 秒，整次刷新仍有兜底（失败保留原缓存，与段永平一侧一致）。实测缓存 291 → 311 条，最新到 9/17 17:25，正文 / 图片 / 中文翻译都正常；新增回归用例锁住「时间取 time[datetime]」「正文容器带额外属性」「旧版纯文本日期仍可解析」。",
    kind: "fix"
  }, {
    title: "修复交易广场更新延迟与刷新回退到旧帖",
    desc: "两个问题一起修。1) 更新有延迟：段永平 9/17 23:49 的转发帖（vivo 那条）一直没出现在站内。根因是他转发别人的帖子时，自己的正文只有一个 emoji 图片（👍）、也没有自己的图片，而 mapDuanStatus 要求「正文或图片至少有一个」才保留，于是整条被丢掉；同时 emoji 图片此前会被「去标签」步骤删掉，纯表情帖会变成空正文。现在保留条件改成「正文 / 图片 / 引用 有一个就留」，并在去标签前把 emoji 图片的 alt/title 取出来（显示成 [很赞] 这种雪球原生写法）。实测缓存 278 → 279 条，最新即为 9/17 23:49 那条，引用内容完整。2) 刷新会先显示旧帖再跳回新的：客户端挂载时把 SSR 首帧下发的帖子整份替换成 localStorage 缓存，而本地缓存必然不新于服务端（它本来就来自服务端），于是「刷新先看到更旧的列表，等接口回来再跳到新的」。现在改为合并：服务端首帧优先，本地缓存只用来补服务端窗口之外的历史。回归用例锁住「转发+引用型帖子不能被丢弃」「emoji 图片要取 alt」「真正空白的状态仍然丢弃」。",
    kind: "fix"
  }, {
    title: "新推文加「新」角标，一眼看到更新",
    desc: "交易广场此前只有作者卡片上的未读数字，列表里看不出哪几条是新更新的。现在每条新动态的标题行右侧会带一个绿色「新」角标，判断口径与作者卡片的未读数完全同源（都是「比上次看到的时间新」，已抽到 lib/tradingSquareSeen.ts 用同一个纯函数，两处不会再不一致）；角标按「本次进入页面时的已读快照」计算，所以浏览过程中不会突然消失，下次打开页面若没有更新的帖子就自然不带角标。顺手打磨评论区细节：评论时间改成主流社交写法（刚刚 / N 分钟前 / N 小时前 / 昨天 HH:MM / MM-DD HH:MM，悬停仍可看完整时间），点赞前加拇指图标，点开的「评论 N」按钮会高亮，二级评论页去掉多余的内层滚动条（跟着页面一起滚）。回归用例覆盖「未读判断与作者计数同源（新 / 旧 / 从未看过 / 脏时间 / 首次访问）」与相对时间五档。",
    kind: "feature"
  }, {
    title: "新动态角标重做：站内未读色的小胶囊，不再用离线绿色块",
    desc: "上一版把「新」做成实心绿胶囊并用 ml-auto 顶在标题行最右端，两处都不对：颜色 #4caf58 是站内没有的绿（本站绿色表示下跌，红才是「新 / 未读」—— 头像上的未读计数一直用的就是 up），而且被推到行尾后与发布时间之间空出一大段，像临时贴上去的。现在改成站内未读色的小胶囊（浅色 bg-up-bg 底 + text-up 字，深色 #3a1c1c 底 + #ff8a8a 字，与订单页的买入标记同一套 token），紧紧跟在发布时间后面，标题行读下来就是「谁 · 什么时候 · 新」。用无头 Chrome 按真实 token 渲染了深浅色对照图确认：浅色 #fdeeee 底红字、深色 #3a1c1c 底 #ff8a8a 字，行内不再有异常留白。回归用例补一条「角标必须用站内未读色、不得再出现 #4caf58」。tsc 无错误、npm run test:review 全通过。",
    kind: "fix"
  }, {
    title: "新动态改为主流做法：单条小圆点 + 交界分隔线",
    desc: "此前给每条新帖贴一枚文字角标，翻了一圈主流站点其实都不这么做：X / Bluesky / Threads 是列表顶部给一枚「N 条新帖子」胶囊（点它才插入新帖），Slack / Discord / Inoreader 是在「已读 / 未读」交界处画一条带小标签的细分隔线，Gmail / Feedly / Inoreader 的单条未读则只用一个圆点、标题略重，Reddit / HN 干脆靠「已访问」变色。现在按后两类改成：1) 每条新动态在最前面加一个 6px 圆点（浅色 #e23d3d、深色 #ff8a8a，与头像未读计数同色），像时间后面贴的红色文字胶囊去掉了；2) 新动态与上次看过的帖子交界处画一次细分隔线，标注「以上 N 条为新动态」，一条线就把两段分干净，不用逐条贴标记；3) 左侧作者卡片上的未读数保留 —— 那对应阅读器的「每个订阅源未读数」，是这套体系里该有的汇总。分界逻辑抽到 lib/tradingSquareSeen.ts 的 unseenBoundaryIndex 纯函数：整页都是新动态（第一次打开）或全看过时不画线，混合时只取最下面那条分界，避免出现多条线。回归用例覆盖分界下标（两条新动态 / 无新动态 / 全为新 / 空列表）与标记形态（必须是小圆点 + 分隔线，且不得再引入站外的绿色）。tsc 无错误、npm run test:review 33 个套件全通过。",
    kind: "fix"
  }, {
    title: "未读分界标签改压到帖间分隔线上，去掉了红线和红字",
    desc: "上一版的分界用的是「左对齐红字 + 一条横贯整行的红细线」，在深色模式下它夹在帖间两条分隔线之间，看着像一道脏边。现在改用微信「以下是新消息」/ Telegram「Unread messages」的写法：不新增任何线条，只把一行 10px 中性灰小字居中压在「本来就存在的那条帖间分隔线」中间（容器高度为 0，标签用卡片底色把线打断），扫列表时依旧一眼能看到新动态到哪儿结束，但不再多出任何一道颜色。红色只保留在单条未读圆点上，分界处不出现第二种红。回归用例改为断言「分界标签是中性灰小字」且「压在原有分隔线上」。tsc 无错误、npm run test:review 33 个套件全通过。",
    kind: "fix"
  }, {
    title: "修复深色模式下左侧导航滚动条默认常显",
    desc: "按约定左侧导航的滚动条应当默认隐藏、鼠标划过才出现，实际在深色模式下一直露着。排查结论：左侧导航（.fire-sidebar-panel）的「默认透明」只写了一个类名（特异性 0-1-0），后来加的「全站滚动条深色自动换色」用的是 `.dark *`（同样是 0-1-0）且位置更靠后，深色模式下就把默认透明盖成了白色 26%，滚动条于是常显；浅色模式没有这条覆盖，所以一直是正常的。修复：默认隐藏的规则补上 `.dark` 前缀（`.fire-sidebar-panel,.dark .fire-sidebar-panel{scrollbar-color:transparent transparent}`，webkit 分支同样处理），悬停 / 聚焦显示的两条规则特异性更高（0-2-0 / 0-3-0），不受影响。验证用无头 Chrome 加载真实样式表实测计算样式：修复前深色下 scrollbar-color 为 rgba(255,255,255,0.26)，修复后为 rgba(0,0,0,0)；再用调试协议把鼠标移上去，确认悬停时恢复成 rgba(255,255,255,0.26)、移开后回到透明，元素 :hover 状态同步为 true。AGENTS.md 滚动条规范补了一条「默认隐藏必须带 .dark 前缀」的约定，回归用例改为断言这条规则与悬停规则同时存在。tsc 无错误、npm run test:review 34 个套件全通过。",
    kind: "fix"
  }]
};
