...
(async () => {
  ...
  await test('card wander shuffles deterministically and preserves the reference wall angle', () => {
    ...
    assert(['光泽', '幻彩', '金属', '脉冲', '星砂'].every((label) => wander.includes(`label: "${label}"`)), 'all five card effects stay selectable');
    ...
    assert(wanderStyles.includes('.card-wander-effects { grid-template-columns: repeat(5, 1fr); }') && wanderStyles.includes('@keyframes card-wander-effect-pulse') && wanderStyles.includes('@keyframes card-wander-effect-stardust'), 'the five-way selector and two restrained effects remain available');
