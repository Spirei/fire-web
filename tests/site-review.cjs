...
(async () => {
  await test('card wander shuffles deterministically and preserves the reference wall angle', () => {
    ...
    assert(wander.includes('setZoomEffect(PREVIEW_EFFECTS[Math.floor(Math.random() * PREVIEW_EFFECTS.length)].key)') && wander.includes('openZoom();') && wander.includes('className="card-wander-zoom-card" data-effect={zoomEffect}') && wanderStyles.includes('.card-wander-zoom-card[data-effect="gloss"] .card-wander-effect-sheen') && wanderStyles.includes('.card-wander-zoom-card[data-effect="holo"] .card-wander-effect-sheen') && wanderStyles.includes('.card-wander-zoom-card[data-effect="metal"] .card-wander-effect-sheen'), 'each enlarged card randomly animates one of the three effects over its original image');
