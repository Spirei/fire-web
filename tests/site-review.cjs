...
(async () => {
  ...
  await test('card wander shuffles deterministically and preserves the reference wall angle', () => {
    ...
    assert(library.includes('loading={index < 4 ? "eager" : "lazy"}') && library.includes('src={wanderSeed ? undefined : cardCover') && !library.includes('decoding={index < PAGE_SIZE_FIRST ? "sync"'), 'library limits eager decoding and releases covered images');
