...
(async () => {
  await test('card wander shuffles deterministically and preserves the reference wall angle', () => {
    ...
    assert(wander.includes('animation.finished.then(() => {') && !wander.includes('landTimer') && !wanderStyles.includes('transform: scale(.96);') && wanderStyles.includes('.card-wander-preview-stage { opacity: 0; }') && wanderStyles.includes('.card-wander-modal[data-landed]:not([data-closing]) .card-wander-effect:not([data-pointer-active])'), 'preview image and idle effect start only after the flight lands without resizing its panel');
