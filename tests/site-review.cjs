...
(async () => {
  await test('card wander shuffles deterministically and preserves the reference wall angle', () => {
    ...
    assert(['光泽', '幻彩', '金属', '珠光', '星砂'].every((label) => wander.includes(`label: "${label}"`)), 'all five card effects stay selectable');
    ...
    assert(wander.includes('setZoomEffect(PREVIEW_EFFECTS[Math.floor(Math.random() * PREVIEW_EFFECTS.length)].key)') && wander.includes('openZoom();') && wander.includes('className="card-wander-zoom-card" data-effect={zoomEffect}') && ['gloss', 'holo', 'metal', 'pearl', 'stardust'].every((effect) => wanderStyles.includes(`.card-wander-zoom-card[data-effect="${effect}"] .card-wander-effect-sheen`)), 'each enlarged card randomly animates one of the five effects over its original image');
    assert(wanderStyles.includes('.card-wander-effects { grid-template-columns: repeat(5, 1fr); }') && wanderStyles.includes('@keyframes card-wander-effect-pearl') && wanderStyles.includes('@keyframes card-wander-effect-stardust'), 'the five-way selector and two restrained effects remain available');
