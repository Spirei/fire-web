...
(async () => {
  await test('card wander shuffles deterministically and preserves the reference wall angle', () => {
    ...
    assert(wander.includes('zoomEffectBagRef.current.pop()') && wander.includes('previousZoomEffectRef.current = effect') && wander.includes('openZoom();') && wander.includes('className="card-wander-zoom-card" data-effect={zoomEffect}') && ['gloss', 'holo', 'metal', 'pearl', 'stardust'].every((effect) => wanderStyles.includes(`.card-wander-zoom-card[data-effect="${effect}"] .card-wander-effect-sheen`)), 'the five enlarged-card effects play in shuffled cycles without immediate repetition');
